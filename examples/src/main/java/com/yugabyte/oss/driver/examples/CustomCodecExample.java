    ...
    try {
      ...
      CqlSession session =
          CqlSession.builder()
              ...
              .addTypeCodecs(new DoubleListToArrayCodec(), new LongListToArrayCodec())
              .build();
      ...
      String createTable =
          "create table  ybdemo.customcodectest(id int Primary key, doublelist list<double>, longlist list<bigint>);";
      session.execute(createTable);
      System.out.println("Created table customcodectest");

      // Insert a row.
      String insert =
          "insert into ybdemo.customcodectest(id, doublelist, longlist) values (1, [1.0,2.0], [123456789, 987654321]);";
      ...
      for (int i = 0; i < doublelist.length; i++) System.out.print(doublelist[i] + ",");

      System.out.println("\nLong list returned:");
      for (int i = 0; i < longlist.length; i++) System.out.print(longlist[i] + ",");
      // Close the client.
      session.close();
    } catch (Exception e) {
