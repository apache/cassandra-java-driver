...
public class Generators {
  ...
  public static final QueryGenerator.Builder INSERTER =
      new QueryGenerator.Builder() {
        ...
        public OptionParser addOptions(OptionParser parser) {
          String msg =
              "Simple insertion of CQL3 rows (using prepared statements unless the --no-prepare option is used). "
          ...
          parser
              .accepts("columns-per-row", "Number of columns per CQL3 row")
              .withRequiredArg()
              .ofType(Integer.class)
              .defaultsTo(5);
          parser
              .accepts("value-size", "The size in bytes for column values")
              .withRequiredArg()
              .ofType(Integer.class)
              .defaultsTo(34);
        ...
        public void prepare(OptionSet options, Session session) {
          ...
          session.execute(
              "CREATE KEYSPACE stress WITH replication = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 }");
          ...
          if (options.has("with-compact-storage")) sb.append(" WITH COMPACT STORAGE");
        ...
        public QueryGenerator createRegular(
            int id, int iterations, OptionSet options, final Session session) {
            ...
            public QueryGenerator.Request next() {
              ...
              for (int i = 0; i < columnsPerRow; ++i) {
                ...
                sb.append('C')
                    .append(i)
                    .append("='")
                    .append(Bytes.toHexString(makeValue(valueSize)))
                    .append('\'');
              }
        ...
        public QueryGenerator createPrepared(
            int id, int iterations, OptionSet options, Session session) {
          final int valueSize = (Integer) options.valueOf("value-size");
          final int columnsPerRow = (Integer) options.valueOf("columns-per-row");
          final long prefix = (long) id << 32;
          ...
          return new AbstractGenerator(iterations) {
            ...
            public QueryGenerator.Request next() {
              ...
              for (int i = 0; i < columnsPerRow; ++i) b.setBytes("c" + i, makeValue(valueSize));
              ++iteration;
  ...
  public static final QueryGenerator.Builder READER =
      new QueryGenerator.Builder() {
        ...
        public OptionParser addOptions(OptionParser parser) {
          String msg =
              "Read the rows inserted with the insert generator. Use prepared statements unless the --no-prepare option is used.";
          parser.formatHelpWith(Stress.Help.formatFor(name(), msg));
        ...
        public void prepare(OptionSet options, Session session) {
          ...
          if (ks == null || ks.getTable("standard1") == null) {
            System.err.println(
                "There is nothing to reads, please run insert/insert_prepared first.");
            System.exit(1);
          }
