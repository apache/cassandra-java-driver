...
/**
 * Unified test for SimpleStatement that can run against either Cassandra (via CCM) or Astra.
 *
 * <p>The backend is selected via the {@code ccm.distribution} system property, which is handled by
 * {@link CassandraResourceRuleFactory}.
 *
...
public class SimpleStatementIT {
  ...
  @Test
  public void should_execute_query_async() throws Exception {
    Statement<?> statement =
        SimpleStatement.builder(String.format("SELECT v FROM test WHERE k='%s'", KEY)).build();

    CompletionStage<AsyncResultSet> future = SESSION_RULE.session().executeAsync(statement);
