    ...
    for (DefaultSessionMetric metric : ENABLED_SESSION_METRICS) {
      ...
      switch (metric) {
        ...
        case CQL_REQUESTS:
          ...
          assertThat(((Timer) m).count()).isEqualTo(63);
          break;
        case CQL_CLIENT_TIMEOUTS:
          assertThat(m).isInstanceOf(Counter.class);
          assertThat(((Counter) m).count()).isEqualTo(3);
          break;
    ...
    for (Node node : session.getMetadata().getNodes().values()) {
      ...
      for (DefaultNodeMetric metric : ENABLED_NODE_METRICS) {
        ...
        switch (metric) {
          ...
          case CQL_MESSAGES:
            assertThat(m).isInstanceOf(Timer.class);
            assertThat(((Timer) m).count()).isEqualTo(28);
            break;
          case READ_TIMEOUTS:
          case WRITE_TIMEOUTS:
          case UNAVAILABLES:
          ...
          case IGNORES:
            assertThat(m).isInstanceOf(Counter.class);
            assertThat(((Counter) m).count()).isEqualTo(5);
            break;
          ...
          case SPECULATIVE_EXECUTIONS:
            assertThat(m).isInstanceOf(Counter.class);
            assertThat(((Counter) m).count()).isOne();
            break;
