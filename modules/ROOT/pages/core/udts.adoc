...
* getting hold of the https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/type/UserDefinedType.html[UserDefinedType]:
...
The driver maps UDT columns to the https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/data/UdtValue.html[UdtValue] class, which exposes getters and setters to access individual fields by index or name:
...
To create a new UDT value, you must first have a reference to its https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/type/UserDefinedType.html[UserDefinedType].
...
* from the driver's xref:core/schemaMetadata[schema metadata]:
+
[source,java]
----
  UserDefinedType udt =
...
Note that the driver's official API does not expose a way to build https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/type/UserDefinedType.html[UserDefinedType] instances manually.
...
There is still a way to do it with the driver, but it's part of the xref:core/apiConventions.adoc[internal API]:
...
Note that a manually created type is xref:core/detachableTypes.adoc[detached].
