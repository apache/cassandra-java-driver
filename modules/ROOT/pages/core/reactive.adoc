...
* The reactive execution model is implemented in a non-blocking fashion: see the manual page on  xref:core/nonBlocking.adoc[non-blocking programming] for details.
...
As a last resort, it is also possible to limit concurrency at driver level, for example using the driver's built-in xref:core/throttling.adoc[request throttling] mechanism, although this is usually not required in reactive applications.
...
Note that the driver already has a xref:core/retries.adoc[built-in retry mechanism] that can transparently retry failed queries;
