...
* xref:core/pooling.adoc[connection pools]: for each node, a session has a fixed-size pool of connections to execute user requests.
...
* xref:core/controlConnection.adoc[control connection]: a session uses a single connection to an arbitrary node for administrative requests.
...
https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/internal/core/connection/ExponentialReconnectionPolicy.html[ExponentialReconnectionPolicy] is the default;
it starts with a base delay, and then doubles it after each attempt.
https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/internal/core/connection/ConstantReconnectionPolicy.html[ConstantReconnectionPolicy] uses the same delay every time, regardless of the previous number of attempts.
...
It invokes the xref:core/loadBalancing.adoc[load balancing policy] to get a query plan, which happens to start with node4.
