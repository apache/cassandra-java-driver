...
xref:core/pooling.adoc[Connection pools] can be sized independently for each distance.
...
The driver then tries each node in sequence, moving down the plan according to the xref:core/retries.adoc[retry policy] and xref:core/speculativeExecution.adoc[speculative execution policy].
...
The actual number of connections to open to each one of those nodes is configurable, see xref:core/pooling,adoc[Connection pools] for more details.
...
First make sure that xref:core/metadataToken.adoc#configuration[token metadata] is enabled.

Then your statements need to provide:

* a keyspace: if you use a xref:core/statements/perQueryKeyspace.adoc[per-query keyspace], then it will be used for routing as well.
Otherwise, the driver relies on https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/session/Request.html#getRoutingKeyspace--[getRoutingKeyspace()];
...
For xref:core/statements/simpleStatements.adoc[simple statements], routing information is never computed automatically:
...
For xref:core/statements/preparedStatements.adoc[bound statements], the keyspace is always available;
...
For xref:core/statements/batchStatements.adoc[batch statements], the routing information of each child statement is inspected;
...
Study the https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/loadbalancing/LoadBalancingPolicy.html[LoadBalancingPolicy] interface and the built-in [BasicLoadingBalancingPolicy] for the low-level details.
...
The load balancing policy can be overridden in xref:core/configuration.adoc#profiles[execution profiles]:
