...
* xref:core/retries.adoc[retries]: if we're waiting for a response from a node and the connection gets dropped, the default retry policy automatically retries on another node.
...
* xref:core/speculativeExecution.adoc[speculative executions]: if they are enabled and a node takes too long to respond, the driver queries another node to get the response faster.
...
refer to xref:queryBuilder/idempotence.adoc[its manual] for more details.
