...
xref:core/statements/index.adoc[Statements] are some of the driver types you'll use the most.
...
xref:core/statements/preparedStatements.adoc[Prepared statements] allow Cassandra to cache parsed query strings server-side, but that's not their only benefit for performance:
...
The driver caches prepared statements -- see https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/CqlSession.html#prepare-com.datastax.oss.driver.api.core.cql.SimpleStatement-[CqlSession.prepare(SimpleStatement)] for the fine print.
...
See the xref:core/pooling.adoc#tuning[tuning] section in the connection pooling page.

=== Compression

Consider xref:core/compression.adoc[compression] if your queries return large payloads;
it might help to reduce network traffic.

=== Timestamp generation

Each query is assigned a xref:core/queryTimestamps.adoc[timestamp] to order them relative to each other.

By default, this is done driver-side with xref:core/queryTimestamps.adoc#atomic-timestamp-generator[AtomicTimestampGenerator].
This is a very simple operation so unlikely to be a bottleneck, but note that there are other options, such as a xref:core/queryTimestamps.adoc#thread-local-timestamp-generator[thread-local] variant that creates slightly less contention, writing your own implementation or letting the server assign timestamps.

=== Tracing

xref:core/tracing.adoc[Tracing] should be used for only a small percentage of your queries.
...
xref:core/requestTracker.adoc[Request trackers] are on the hot path (that is, invoked on I/O threads, each time a request is executed), and users can plug custom implementations.
...
xref:core/throttling.adoc[Throttling] can help establish more predictable server performance, by controlling how much load each driver instance is allowed to put on the cluster.
...
The driver uses https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/CqlIdentifier.html[CqlIdentifier] to deal with xref:core/caseSensitivity[case sensitivity].
...
https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/type/reflect/GenericType.html[GenericType] is used to express complex generic types -- such as xref:core/index.adoc#collection-types[nested collections] -- in getters and setters.
...
Similarly, xref:queryBuilder/index.adoc[built queries] are immutable and don't need a reference to a live driver instance.
...
The driver maintains xref:core/metadata.adoc[metadata] about the state of the Cassandra cluster.
...
* if the token map is disabled, `session.getMetadata().getTokenMap()` will always be empty, and you lose the ability to use xref:core/loadBalancing.adoc#token-aware[token-aware routing].
...
Perhaps more interestingly, metadata can be xref:core/schemaMetadata.adoc#filtering[filtered] to a specific subset of keyspaces.
...
To get a sense of the time spent on metadata refreshes, enable xref:core/logging.adoc[debug logs] and look for entries like this:
...
If you have multiple client instances, it might be a good idea to xref:core/schemaMetadata.adoc#enabling-disabling[deactivate the metadata] on all clients while you apply the updates, and reactivate it at the end (reactivating will trigger an immediate refresh, so you might want to ramp up clients to avoid a "thundering herd" effect).
...
The driver handles this automatically by checking for lxref:core/schemaMetadata.adoc#schema-agreement[schema agreement] after each DDL query.
...
This is one of the rare cases where we recommend using https://docs.datastax.com/en/drivers/java/4.13/com/datastax/oss/driver/api/core/cql/Statement.html#setNode-com.datastax.oss.driver.api.core.metadata.Node-[Statement.setNode()].
...
* the *cold path* is for all administrative tasks: managing the xref:core/controlConnection.adoc[control connection], parsing xref:core/metadata.adoc[metadata], reacting to cluster events (node going up/down, getting added/removed, etc), and scheduling periodic events (reconnections, reloading the configuration).
...
Note that you can gain more fine-grained control over thread pools via the xref:core/apiConventions.adoc[internal] API (look at the `NettyOptions` interface).
