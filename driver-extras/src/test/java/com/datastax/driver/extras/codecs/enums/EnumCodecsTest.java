...
/**
 * A test that validates that Enums are correctly mapped to varchars (with EnumNameCodec) and ints
 * (with EnumOrdinalCodec). It also validates that both codecs may coexist in the same
 * CodecRegistry.
 */
...
public class EnumCodecsTest extends CCMTestsSupport {
  ...
  private final String insertQuery =
      "INSERT INTO t1 (pk, foo, foos, bar, bars, foobars, tup, udt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  public void onTestContextInitialized() {
    execute(
        "CREATE TYPE IF NOT EXISTS udt1 (" + "foo int," + "bar text)",
  ...
  public void before() {
    ...
    tupleValue = tup.newValue().set(0, FOO_1, Foo.class).set(1, BAR_1, Bar.class);
    UserType udt = cluster().getMetadata().getKeyspace(keyspace).getUserType("udt1");
    udtValue = udt.newValue().set("foo", FOO_1, Foo.class).set("bar", BAR_1, Bar.class);
  }
  ...
  public void should_use_enum_codecs_with_prepared_statements_1() {
    session()
        .execute(
            session()
                .prepare(insertQuery)
                .bind(pk, FOO_1, foos, BAR_1, bars, foobars, tupleValue, udtValue));
  ...
  private void assertRow(Row row) {
    ...
    assertThat(row.getObject(1))
        .isEqualTo(FOO_1.ordinal()); // uses the built-in IntCodec because CQL type is int
    ...
    assertThat(row.getObject(2))
        .isEqualTo(
            newArrayList(
                FOO_1.ordinal(),
                FOO_2.ordinal())); // uses the built-in ListCodec(IntCodec) because CQL type is
    // list<int>
    assertThat(row.getList(2, Integer.class))
    ...
    assertThat(row.get(2, TypeTokens.listOf(Integer.class)))
        .isEqualTo(newArrayList(FOO_1.ordinal(), FOO_2.ordinal()));
    assertThat(row.get("foos", TypeTokens.listOf(Foo.class))).isEqualTo(newArrayList(FOO_1, FOO_2));

    assertThat(row.getObject(3))
        .isEqualTo(BAR_1.name()); // uses the built-in VarcharCodec because CQL type is varchar
    ...
    assertThat(row.getObject(4))
        .isEqualTo(
            newHashSet(
                BAR_1.name(),
                BAR_2.name())); // uses the built-in SetCodec(VarcharCodec) because CQL type is
    // set<varchar>
    ...
    assertThat(row.get(4, TypeTokens.setOf(String.class)))
    ...
    // uses default built-in codec MapCodec(IntCodec, VarcharCodec) because CQL type is map<int,
    // varchar>
    assertThat(row.getObject(5))
    ...
    assertThat(row.getMap(5, Integer.class, String.class))
        .isEqualTo(ImmutableMap.of(FOO_1.ordinal(), BAR_1.name(), FOO_2.ordinal(), BAR_2.name()));
    assertThat(row.getMap(5, Foo.class, String.class))
        .isEqualTo(ImmutableMap.of(FOO_1, BAR_1.name(), FOO_2, BAR_2.name()));
    assertThat(row.getMap(5, Integer.class, Bar.class))
        .isEqualTo(ImmutableMap.of(FOO_1.ordinal(), BAR_1, FOO_2.ordinal(), BAR_2));
    assertThat(row.getMap(5, Foo.class, Bar.class))
        .isEqualTo(ImmutableMap.of(FOO_1, BAR_1, FOO_2, BAR_2));

    // get + TypeToken with combinations of built-in or EnumX codecs
    assertThat(row.get("foobars", TypeTokens.mapOf(Integer.class, String.class)))
        .isEqualTo(ImmutableMap.of(FOO_1.ordinal(), BAR_1.name(), FOO_2.ordinal(), BAR_2.name()));
    assertThat(row.get("foobars", TypeTokens.mapOf(Foo.class, String.class)))
        .isEqualTo(ImmutableMap.of(FOO_1, BAR_1.name(), FOO_2, BAR_2.name()));
    assertThat(row.get("foobars", TypeTokens.mapOf(Integer.class, Bar.class)))
        .isEqualTo(ImmutableMap.of(FOO_1.ordinal(), BAR_1, FOO_2.ordinal(), BAR_2));
    assertThat(row.get("foobars", TypeTokens.mapOf(Foo.class, Bar.class)))
        .isEqualTo(ImmutableMap.of(FOO_1, BAR_1, FOO_2, BAR_2));
  ...
  public static class Mapped {
    ...
    @PartitionKey private int pk;

    @ClusteringColumn private Foo foo;
