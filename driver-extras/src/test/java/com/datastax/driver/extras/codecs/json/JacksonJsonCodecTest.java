...
public class JacksonJsonCodecTest extends CCMTestsSupport {
  ...
  public void should_use_custom_codec_with_built_statements_2() {
    BuiltStatement insertStmt =
        insertInto("t1").value("c1", notAJsonString).value("c2", alice).value("c3", bobAndCharlie);
    BuiltStatement selectStmt =
        select("c1", "c2", "c3").from("t1").where(eq("c1", notAJsonString)).and(eq("c2", alice));
  ...
  public static class User {
    ...
    public boolean equals(Object o) {
      ...
      User user = (User) o;
      return MoreObjects.equal(id, user.id) && MoreObjects.equal(name, user.name);
    }
