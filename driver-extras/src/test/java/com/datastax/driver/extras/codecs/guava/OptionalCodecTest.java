...
public class OptionalCodecTest extends CCMTestsSupport {
  ...
  private final OptionalCodec<List<String>> optionalCodec =
      new OptionalCodec<List<String>>(list(varchar()));
  ...
  /**
   * Validates that if a column is unset, that retrieving the value using {@link OptionalCodec}
   * should return an {@link Optional#absent()} value. Since CQL Lists can't differentiate between
   * null and empty lists, the OptionalCodec should be smart enough to map an empty list to absent.
   *
  ...
  /**
   * Validates that if a column is set to {@link Optional#absent()} using {@link OptionalCodec} that
   * it should be stored as null and that retrieving it should return {@link Optional#absent()}
   * using {@link OptionalCodec}.
   *
   * @test_category data_types:serialization
   * @expected_result an absent value.
   * @jira_ticket JAVA-846
  ...
  public void should_map_absent_null_value_to_absent() {
    ...
    ResultSet results =
        session().execute(selectPrep.bind("should_map_absent_null_value_to_absent"));
    assertThat(results.getAvailableWithoutFetching()).isEqualTo(1);
  ...
  /**
   * Validates that if a column is set to an {@link Optional} value using {@link OptionalCodec} that
   * it should be stored as the option's value and that retrieving it should return an {@link
   * Optional} using {@link OptionalCodec} and its actual value without using it.
   *
   * @test_category data_types:serialization
   * @expected_result The options value is stored appropriately and is retrievable with and without
   *     OptionalCodec.
   * @jira_ticket JAVA-846
   * @since 2.2.0
   */
  ...
  public void should_map_a_primitive_type_to_absent() {
    ...
    ResultSet results =
        session().execute(selectBigint.bind("should_map_a_primitive_type_to_absent"));

    assertThat(results.getAvailableWithoutFetching()).isEqualTo(1);

    Row row = results.one();
    assertThat(row.get("c4", optionalLongCodec.getJavaType())).isEqualTo(Optional.<Long>absent());
    assertThat(row.getLong("c4"))
        .isEqualTo(0L); // This will return a 0L since it returns the primitive value.
  ...
  public void should_map_a_nullable_type_to_absent() {
    ...
    ResultSet results =
        session().execute(selectDecimal.bind("should_map_a_nullable_type_to_absent"));

    assertThat(results.getAvailableWithoutFetching()).isEqualTo(1);

    Row row = results.one();
    assertThat(row.get("c5", optionalDecimalCodec.getJavaType()))
        .isEqualTo(Optional.<BigDecimal>absent());
    assertThat(row.getDecimal("c5"))
        .isNull(); // Since BigDecimal is not a primitive it is nullable so expect null.
  }
}
