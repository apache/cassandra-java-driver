...
public class JodaTimeCodecsTest extends CCMTestsSupport {
  ...
  public void should_map_time_to_localtime() {
    ...
    assertThat(row.getTime("ctime"))
        .isEqualTo(NANOSECONDS.convert(time.getMillisOfDay(), MILLISECONDS));
  }

  /**
   * Validates that a <code>date</code> column can be mapped to a {@link org.joda.time.LocalDate} by
   * using {@link LocalDateCodec}.
   *
   * @test_category data_types:serialization
   * @expected_result properly maps.
   * @jira_ticket JAVA-846
  ...
  public void should_map_timestamp_to_instant() {
    ...
    session()
        .execute(
            ...
            "should_map_timestamp_to_datetime",
            instant);
    ResultSet result =
        session()
            .execute("select ctimestamp from foo where c1=?", "should_map_timestamp_to_datetime");
    // then
    assertThat(result.getAvailableWithoutFetching()).isEqualTo(1);
    Row row = result.one();
   ...
   * DateTime} by using {@link DateTimeCodec}.
   *
   * @test_category data_types:serialization
   * @expected_result properly maps.
   * @jira_ticket JAVA-846
  ...
  public void should_map_tuple_to_datetime() {
    ...
    ResultSet result =
        session().execute("select ctuple from foo where c1=?", "should_map_tuple_to_datetime");
    // then
    assertThat(result.getAvailableWithoutFetching()).isEqualTo(1);
    Row row = result.one();
