 ...
 * efficient implementation of {@link #serialize(Object, ProtocolVersion)} and {@link
 * #deserialize(ByteBuffer, ProtocolVersion)} for primitive arrays.
 *
...
public abstract class AbstractPrimitiveArrayCodec<T> extends AbstractArrayCodec<T> {
  ...
  public AbstractPrimitiveArrayCodec(DataType.CollectionType cqlType, Class<T> javaClass) {
    ...
    checkArgument(
        javaClass.getComponentType().isPrimitive(),
        "Expecting primitive array component type, got %s",
        javaClass.getComponentType());
  }

  @Override
  public ByteBuffer serialize(T array, ProtocolVersion protocolVersion)
    ...
    checkArgument(
        isProtocolV3OrAbove || length < 65536,
  ...
  protected abstract void serializeElement(
      ByteBuffer output, T array, int index, ProtocolVersion protocolVersion);
  ...
  protected abstract void deserializeElement(
      ByteBuffer input, T array, int index, ProtocolVersion protocolVersion);
}
