...
public class ConstantSpeculativeExecutionPolicy implements SpeculativeExecutionPolicy {
  ...
  public ConstantSpeculativeExecutionPolicy(DriverContext context, DriverOption configRoot) {
    DriverConfigProfile config = context.config().getDefaultProfile();
    this.maxExecutions =
        config.getInt(configRoot.concat(CoreDriverOption.RELATIVE_SPECULATIVE_EXECUTION_MAX));
    if (this.maxExecutions < 1) {
      throw new IllegalArgumentException("Max must be at least 1");
    }
    this.constantDelayMillis =
        config
            .getDuration(configRoot.concat(CoreDriverOption.RELATIVE_SPECULATIVE_EXECUTION_DELAY))
            .toMillis();
