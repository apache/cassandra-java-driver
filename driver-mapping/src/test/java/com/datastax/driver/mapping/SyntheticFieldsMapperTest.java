...
public class SyntheticFieldsMapperTest extends CCMTestsSupport {
   ...
   *
   * <p>This test creates a modified version of this class where {@code futureSynthetic} is marked
   * as synthetic, and therefore ignored by the {@code AnnotationParser}. If the test throws an
   * error "Cannot find matching getter and setter for field 'futureSynthetic'", it means that the
   * field is not ignored by the mapped. However, if it succeed, we know that the field was ignored
   * and that the fix does the right job.
   *
   * <p>If the class {@code ClassWithSyntheticField} was used as-is, the {@code Mapper} would
   * complain that there are no getter and setter for the field {@code futureSynthetic}.
   *
   * <p>Note that we could also have used not-static inner classes, but the {@code Mapper} is never
   * able to instantiate those as they require a reference to their enclosing class that is not
  ...
  public void should_ignore_synthetic_fields() {
    ...
    // Here we cannot use ClassWithSyntheticField since it is not the one the source file was
    // compiled against
    // Instead, it is a different Class (identity + ClassLoader), a modified version of
    // ClassWithSyntheticField
    // Nice ClassCastException will be thrown if we try to cast it to ClassWithSyntheticField
    @SuppressWarnings("unchecked")
    Mapper<Object> m =
        (Mapper<Object>) new MappingManager(session()).mapper(classWithSyntheticFields);
    m.save(instance);
  ...
  private Class<?> makeTestClassWithSyntheticFields() {
    ...
    try {
      ...
      // Build the new class
      return new InterceptingClassLoader()
          .defineClass(ClassWithSyntheticField.class.getName(), updatedClassBytes);
    } catch (IOException e) {
  ...
  private Object instantiateNewClass(Class<?> classWithSyntheticFields, int id) {
    try {
      Constructor<?> declaredConstructor =
          classWithSyntheticFields.getDeclaredConstructor(int.class);
