...
public class MapperSaveNullFieldsTest extends CCMTestsSupport {
  ...
  /**
   * ensure that queries generated between different sessions consistently generates the same
   * prepared statement query for different permutations of null columns being present and saved.
   *
  ...
  public void should_order_save_query_prepared_statement_columns_consistently() {
    ...
    for (int i = 0; i < statementCount; i++) {
      ...
      assertThat(uniqueStatements)
          .as("Expected only one statement permutation, must not be ordered consistently.")
          .hasSize(1);
    }
  }

  private String getQuery(
      Mapper<User> mapper, boolean nullName, boolean nullPhone, Option... options) {
    ...
    return ((BoundStatement) mapper.saveQuery(newUser, options))
        .preparedStatement()
        .getQueryString();
  }
  ...
  private void should_save_null_fields(
      boolean nullName, boolean nullPhone, boolean saveExpected, Option... options) {
    // Start with clean data
    session()
        .execute(
            "insert into user(login, name, phone) "
                + "values ('test_login', 'previous_name', 'previous_phone')");

    boolean unsetSupported =
        session().getCluster().getConfiguration().getProtocolOptions().getProtocolVersion().toInt()
            >= 4;
    ...
    String expectedName = nullName ? (saveExpected ? null : "previous_name") : "new_name";
    String expectedPhone = nullPhone ? (saveExpected ? null : "previous_phone") : "new_phone";
