...
public class MappingConfigurationTransientTest extends CCMTestsSupport {
  ...
  public void should_ignore_property_if_field_annotated_transient() {
    ...
    // then mapping should succeed since non-existent column 'notAColumn' is annotated as Transient
    // on its field
  ...
  public void should_ignore_property_if_declared_transient_in_mapper_configuration() {
    ...
    MappingConfiguration conf =
        MappingConfiguration.builder()
            .withPropertyMapper(new DefaultPropertyMapper().addTransientPropertyNames("notAColumn"))
            .build();
    // when creating a mapper
    // then mapping should succeed since non-existent column 'notAColumn' is considered implicitly
    // transient
