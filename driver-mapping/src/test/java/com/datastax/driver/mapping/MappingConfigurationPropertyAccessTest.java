...
public class MappingConfigurationPropertyAccessTest extends CCMTestsSupport {
  ...
  public void should_map_fields_and_getters() {
    ...
    MappingConfiguration conf =
        MappingConfiguration.builder()
            .withPropertyMapper(
                new DefaultPropertyMapper().setPropertyAccessStrategy(PropertyAccessStrategy.BOTH))
            .build();
    MappingManager mappingManager = new MappingManager(session(), conf);
    // when creating a mapper
    // should succeed since fields and getters (k, getV) maps to a C* columns (k, v) and remaining
    // field/getter
  ...
  public static class Foo3 {
    ...
    @Transient private int storeVValueButNotMapped;
