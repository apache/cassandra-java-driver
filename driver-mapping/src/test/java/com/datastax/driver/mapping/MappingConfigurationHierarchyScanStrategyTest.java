...
/**
 * Test for JAVA-1310 - validate ability configure ancestor property scanning: - disable - configure
 * max depth ancestor (included or not)
 */
  ...
  public void should_inherit_annotations_up_to_highest_ancestor_excluded() {
    MappingConfiguration conf =
        MappingConfiguration.builder()
            .withPropertyMapper(
                new DefaultPropertyMapper()
                    .setHierarchyScanStrategy(
                        new DefaultHierarchyScanStrategy(GrandParent2.class, false)))
            .build();
    MappingManager mappingManager = new MappingManager(session(), conf);
    ...
    MappingConfiguration conf =
        MappingConfiguration.builder()
            .withPropertyMapper(
                new DefaultPropertyMapper()
                    .setHierarchyScanStrategy(
                        new DefaultHierarchyScanStrategy(Parent2.class, true)))
            .build();
    MappingManager mappingManager = new MappingManager(session(), conf);
    Mapper<Child2> mapper = mappingManager.mapper(Child2.class);
    assertThat(mapper.get(1).getV()).isEqualTo(1);
