...
class MappedUDTCodec<T> extends TypeCodec.AbstractUDTCodec<T> {
  ...
  MappedUDTCodec(
      UserType cqlUserType,
      Class<T> udtClass,
      Map<String, AliasedMappedProperty> columnMappers,
  ...
  protected ByteBuffer serializeField(T source, String fieldName, ProtocolVersion protocolVersion) {
    ...
    if (aliasedMappedProperty == null) return null;
    ...
    if (codec == null)
      codec =
          codecRegistry.codecFor(
              cqlUserType.getFieldType(aliasedMappedProperty.mappedProperty.getMappedName()),
  ...
  @Override
  protected T deserializeAndSetField(
      ...
      if (codec == null)
        codec =
            codecRegistry.codecFor(
                cqlUserType.getFieldType(aliasedMappedProperty.mappedProperty.getMappedName()),
                aliasedMappedProperty.mappedProperty.getPropertyType());
      aliasedMappedProperty.mappedProperty.setValue(
          target, codec.deserialize(input, protocolVersion));
    }
    return target;
  }
