...
public class Mapper<T> {
    ...
    public Statement updateQuery(T entity) {
        checkNotInEventLoop();
        try {
            return Uninterruptibles.getUninterruptibly(updateQueryAsync(entity, this.defaultSaveOptions));
        } catch (ExecutionException e) {
            throw DriverThrowables.propagateCause(e);
        }
    }
    ...
    public Statement updateQuery(T entity, Option... options) {
        checkNotInEventLoop();
        try {
            return Uninterruptibles.getUninterruptibly(updateQueryAsync(entity, toMapWithDefaults(options, this.defaultSaveOptions)));
        } catch (ExecutionException e) {
            throw DriverThrowables.propagateCause(e);
        }
    }
    ...
    public ListenableFuture<Void> updateAsync(T entity) {
        return submitVoidQueryAsync(updateQueryAsync(entity, this.defaultSaveOptions));
    }
    ...
    public ListenableFuture<Void> updateAsync(T entity, Option... options) {
        return submitVoidQueryAsync(updateQueryAsync(entity, toMapWithDefaults(options, this.defaultSaveOptions)));
    }
    ...
    public static abstract class Option {
        ...
        static class Ttl extends Option {
            ...
            void validate(QueryType qt, MappingManager manager) {
                ...
                checkArgument(qt == QueryType.SAVE, "TTL option is only allowed in save and update queries");
            }
        ...
        static class Timestamp extends Option {
            ...
            void validate(QueryType qt, MappingManager manager) {
                ...
                checkArgument(qt == QueryType.SAVE || qt == QueryType.DEL, "Timestamp option is only allowed in save, update and delete queries");
            }
