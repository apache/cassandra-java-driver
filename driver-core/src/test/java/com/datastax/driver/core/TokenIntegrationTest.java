 ...
 *
 * <p>There's normally a way to parametrize a TestNG class with @Factory and @DataProvider, but it
 * doesn't seem to work with multiple methods.
 */
...
public abstract class TokenIntegrationTest extends CCMTestsSupport {
  ...
  /**
   * Validates that {@link TokenRange}s are exposed via a {@link Cluster}'s {@link Metadata} and
   * they can be used to query data.
   *
  ...
  public void should_expose_token_ranges() throws Exception {
    ...
    // Iterate the cluster's token ranges. For each one, use a range query to ask Cassandra which
    // partition keys
    // are in this range.
   ...
   *
   * <ol>
   *   <li>Retrieve the token for the key with value '1', get it by index, and ensure if is of the
   *       expected token type.
   *   <li>Retrieve the token for the partition key with getPartitionKeyToken
   *   <li>Select data by token with a BoundStatement.
   *   <li>Select data by token using setToken by index.
   *   <li>Select data by token with setPartitionKeyToken.
   * </ol>
   *
   * @test_category token
  ...
  /**
   * Ensures that @{link TokenRange}s are exposed at a per host level, the ranges are complete, the
   * entire ring is represented, and that ranges do not overlap.
   *
   * <p>
   *
   * <p>Also ensures that ranges from another replica are present when a Host is a replica for
   * another node.
   *
   * @test_category metadata:token
   ...
   * <p>
   *
   * <p>Also validates that tokens are not present for multiple hosts.
   *
   * @test_category metadata:token
  ...
  public void should_expose_tokens_per_host() {
    for (Host host : cluster().getMetadata().allHosts()) {
      ...
      // Check against the info in the system tables, which is a bit weak since it's exactly how the
      // metadata is
