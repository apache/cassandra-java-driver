...
public class SchemaRefreshDebouncerTest extends CCMTestsSupport {
   ...
   * that the schema refresh is debounced and coalesced into a single schema refresh for that
   * keyspace only.
   *
   * @throws Exception
   ...
   */
  @Test(groups = "short")
  public void should_debounce_and_coalesce_create_and_alter_keyspace_into_refresh_keyspace()
      throws Exception {
  ...
  /**
   * Ensures that when a CREATED (keyspace) and CREATED (table) schema_change events are received on
   * a control connection with that table belonging to that keyspace within {@link
   * QueryOptions#getRefreshSchemaIntervalMillis()} that the schema refresh is debounced and
   * coalesced into a single schema refresh for that keyspace only.
   *
   * @throws Exception
   * @jira_ticket JAVA-657
   * @since 2.0.11
   */
  @Test(groups = "short")
  public void should_debounce_and_coalesce_create_keyspace_and_table_into_refresh_keyspace()
      throws Exception {
    String keyspace = TestUtils.generateIdentifier("ks_");
    String table = "tbl1";
    session().execute(String.format(CREATE_KEYSPACE_SIMPLE_FORMAT, keyspace, 1));
  ...
  /**
   * Ensures that when multiple CREATED schema_change events are received on a control connection
   * for tables belonging to the same keyspace within {@link
   * QueryOptions#getRefreshSchemaIntervalMillis()} that the schema refresh is debounced and
   * coalesced into a single schema refresh for that keyspace only.
   *
   * @throws Exception
   ...
   */
  @Test(groups = "short")
  public void should_debounce_and_coalesce_tables_in_same_keyspace_into_refresh_keyspace()
      throws Exception {
    String keyspace = TestUtils.generateIdentifier("ks_");
  ...
  /**
   * Ensures that when multiple UPDATED schema_change events are received on a control connection
   * for for the same table within {@link QueryOptions#getRefreshSchemaIntervalMillis()} that the
   * schema refresh is debounced and coalesced into a single schema refresh for that table only.
   *
   * @throws Exception
   * @jira_ticket JAVA-657
   * @since 2.0.11
   */
  @Test(groups = "short")
  public void should_debounce_and_coalesce_multiple_alter_events_on_same_table_into_refresh_table()
      throws Exception {
    ...
    session()
        .execute(String.format("ALTER TABLE %s.%s WITH comment = '%s'", keyspace, table, comment));
    ...
    verify(listener, timeout(DEBOUNCE_TIME * 2).times(1))
        .onTableChanged(captor.capture(), original.capture());
    ...
    verify(controlConnection, times(1))
        .refreshSchema(TABLE, keyspace, table, Collections.<String>emptyList());
   ...
   * on a control connection for different keyspaces within {@link
   * QueryOptions#getRefreshSchemaIntervalMillis()} that the schema refresh is debounced and
   * coalesced into a single full schema refresh.
   *
   * @throws Exception
   * @jira_ticket JAVA-657
   * @since 2.0.11
   */
  @Test(groups = "short")
  public void should_debounce_and_coalesce_multiple_keyspace_creates_into_refresh_entire_schema()
      throws Exception {
    String prefix = TestUtils.generateIdentifier("ks_");
    for (int i = 0; i < 3; i++) {
      session().execute(String.format(CREATE_KEYSPACE_SIMPLE_FORMAT, prefix + i, 1));
      // check that the metadata is immediately up-to-date for the client that issued the DDL
      // statement
      assertThat(cluster().getMetadata().getKeyspace(prefix + i)).isNotNull();
    }

    verify(listener, timeout(DEBOUNCE_TIME * 3).times(3))
        .onKeyspaceAdded(any(KeyspaceMetadata.class));
  ...
  /**
   * Ensures that when enough schema changes have been received on a control connection to reach
   * {@link QueryOptions#getMaxPendingRefreshSchemaRequests()} that a schema refresh is submitted
   * right away.
   *
   * @throws Exception
   * @jira_ticket JAVA-657
  ...
  public void should_refresh_when_max_pending_requests_reached() throws Exception {
    ...
    verify(listener, timeout(DEBOUNCE_TIME * 5).times(5))
        .onKeyspaceAdded(any(KeyspaceMetadata.class));
    // Verify a complete schema refresh was executed, but only once.
    verify(controlConnection, times(1)).refreshSchema(null, null, null, null);
