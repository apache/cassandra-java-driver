...
public class EventDebouncerIntegrationTest extends CCMTestsSupport {
  ...
  /**
   * Tests that DOWN, UP, REMOVE or ADD events will not be delivered to load balancing policy nor
   * host state listeners before the cluster is fully initialized.
   *
  ...
  @Test(groups = "long")
  public void should_wait_until_load_balancing_policy_is_fully_initialized()
      throws InterruptedException {
    ...
    // note: a graceful stop notify other nodes which send a topology change event to the driver
    // right away
