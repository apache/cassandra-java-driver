...
public class SchemaChangesCCTest extends CCMTestsSupport {
   ...
   * <p>
   *
   * <p>Note that on control connection recovery not all schema changes are propagated. For example,
   * if a table was altered and then dropped only a drop event would be received as that is all that
   * can be discerned.
   *
   * @test_category control_connection, schema
   * @expected_result keyspace and table add, drop, and remove events are propagated on control
  ...
  public void should_receive_changes_made_while_control_connection_is_down_on_reconnect()
    ...
    verify(listener, timeout(NOTIF_TIMEOUT_MS).times(2))
    ...
    // Reset the mock to clear invocations. (sanity check to ensure all events happen after CC comes
    // back up)
    reset(listener);
    ...
    while (!cluster.manager.controlConnection.isOpen()
        && System.currentTimeMillis() - startTime < maxControlConnectionWait) {
    ...
    ArgumentCaptor<KeyspaceMetadata> removedKeyspace =
        ArgumentCaptor.forClass(KeyspaceMetadata.class);
    verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
        .onKeyspaceRemoved(removedKeyspace.capture());
    assertThat(removedKeyspace.getValue()).hasName("ks2").isEqualTo(predroppedKeyspace);
    ...
    ArgumentCaptor<KeyspaceMetadata> alteredKeyspace =
        ArgumentCaptor.forClass(KeyspaceMetadata.class);
    ArgumentCaptor<KeyspaceMetadata> originalKeyspace =
        ArgumentCaptor.forClass(KeyspaceMetadata.class);
    verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
        .onKeyspaceChanged(alteredKeyspace.capture(), originalKeyspace.capture());
    ...
    verify(listener, timeout(NOTIF_TIMEOUT_MS).times(1))
        .onTableChanged(alteredTable.capture(), originalTable.capture());
    ...
    ArgumentCaptor<KeyspaceMetadata> addedKeyspace =
        ArgumentCaptor.forClass(KeyspaceMetadata.class);
    ...
    assertThat(addedTable.getValue()).isInKeyspace("ks1").hasName("tbl3");
  }

  /**
   * A load balancing policy that can be "disabled" by having its query plan return no hosts when
   * given the 'DEFAULT' statement. This statement is used for retrieving query plan and for finding
   * what hosts to use for control connection.
   */
