...
public class TokenRangeTest {
  ...
  public void should_merge_with_other_range() {
    ...
    assertThat(tokenRange(3, 9).mergeWith(tokenRange(9, 3)))
        .isEqualTo(tokenRange(minToken, minToken));
    assertThat(tokenRange(3, 9).mergeWith(tokenRange(9, 4)))
        .isEqualTo(tokenRange(minToken, minToken));
    assertThat(tokenRange(3, 10).mergeWith(tokenRange(9, 4)))
        .isEqualTo(tokenRange(minToken, minToken));
    ...
    assertThat(tokenRange(9, 3).mergeWith(tokenRange(3, 9)))
        .isEqualTo(tokenRange(minToken, minToken));
    assertThat(tokenRange(9, 3).mergeWith(tokenRange(3, 10)))
        .isEqualTo(tokenRange(minToken, minToken));

    assertThat(tokenRange(3, 3).mergeWith(tokenRange(3, 3))).isEqualTo(tokenRange(3, 3));

    assertThat(tokenRange(5, minToken).mergeWith(tokenRange(6, 7)))
        .isEqualTo(tokenRange(5, minToken));
    assertThat(tokenRange(5, minToken).mergeWith(tokenRange(minToken, 3)))
        .isEqualTo(tokenRange(5, 3));
    assertThat(tokenRange(5, minToken).mergeWith(tokenRange(3, 5)))
        .isEqualTo(tokenRange(3, minToken));

    assertThat(tokenRange(minToken, 5).mergeWith(tokenRange(2, 3)))
        .isEqualTo(tokenRange(minToken, 5));
    assertThat(tokenRange(minToken, 5).mergeWith(tokenRange(7, minToken)))
        .isEqualTo(tokenRange(7, 5));
    assertThat(tokenRange(minToken, 5).mergeWith(tokenRange(5, 7)))
        .isEqualTo(tokenRange(minToken, 7));
  }
  ...
  public void should_split_evenly() {
    // Simply exercise splitEvenly, split logic is exercised in TokenFactoryTest implementation for
    // each partitioner.
  ...
  public void should_check_if_range_contains_token() {
    ...
    assertThat(tokenRange(2, 1)).contains(minToken, true);

    // ]min, 1] does not contain min, but it contains the start of ]min, 2]
    assertThat(tokenRange(minToken, 1)).doesNotContain(minToken, false).contains(minToken, true);
    // ]1, min] contains min, but not the start of ]min, 2]
    assertThat(tokenRange(1, minToken)).contains(minToken, false).doesNotContain(minToken, true);
