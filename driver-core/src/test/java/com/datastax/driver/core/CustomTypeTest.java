...
public class CustomTypeTest extends CCMTestsSupport {
  ...
  public static final DataType CUSTOM_COMPOSITE =
      custom(
    ...
    try {
      ...
      execute(
          "CREATE TABLE test ("
              ...
              + "    c2 'ReversedType(CompositeType(UTF8Type, Int32Type))'," // reversed translates
              // to CLUSTERING ORDER
              // BY DESC
  ...
  /**
   * Validates that columns using custom types are properly handled by the driver in the following
   * ways:
   *
   * <p>
   *
   * <ol>
   *   <li>The column metadata appropriately represents the types as {@link DataType#custom(String)}
   *   <li>ReversedType is appropriately detected and the clustering order of that column is marked
   *       as descending.
   *   <li>ColumnDefinitions for a column in a {@link Row} matches the custom type and that inserted
   *       data is read back properly.
   * </ol>
   *
  ...
  public void should_serialize_and_deserialize_custom_types() {
    ...
    assertThat(table.getColumn("c2"))
        ...
        .hasClusteringOrder(ClusteringOrder.DESC);
  ...
  /**
   * Validates that UDTs with fields using custom types are properly handled by the driver in the
   * following ways:
   *
   * <p>
   *
   * <ol>
   *   <li>The {@link UserType} metadata appropriately represents the types of fields with custom
   *       types as {@link DataType#custom(String)}
   *   <li>{@link TableMetadata} with a column having a {@link UserType} is properly referenced.
   *   <li>ColumnDefinitions for a column in a {@link Row} matches the {@link UserType} and that
   *       inserted data is read back properly.
   * </ol>
   *
   * @jira_ticket JAVA-993
   * @test_category metadata
  ...
  public void should_handle_udt_with_custom_type() {
    ...
    session()
        .execute(
            "CREATE TYPE custom_udt (regular int, c1 'DynamicCompositeType(s => UTF8Type, i => Int32Type)', c2 'LongType')");
    ...
    // Then: The fields with custom types should be appropriately represented with their defined
    // types.
    ...
    // Then: The column with the user type should be represented and match the type previously
    // retrieved.
    ...
    session()
        .execute(
            "INSERT INTO custom_udt_tbl (k, v) VALUES (0, {regular: 5, c1: 's@hello:i@93', c2: 400})");
