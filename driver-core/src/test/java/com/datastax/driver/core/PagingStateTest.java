...
public class PagingStateTest extends CCMTestsSupport {
  ...
  /**
   * Validates that if the {@link PagingState} is altered in any way that it may not be reused. The
   * paging state is altered in the following ways:
   *
   * <p>
   *
   * <ol>
   *   <li>Altering a byte in the paging state raw bytes.
   *   <li>Setting the {@link PagingState} on a different Statement. (should fail hash validation)
   * </ol>
   *
   * @test_category paging
   * @expected_result {@link PagingState} refused to be reused if it is changed or used on a
   *     different statement.
   */
  @Test(groups = "short")
  ...
  /**
   * Validates if all results of a query are paged in through a queries result set that the {@link
   * PagingState} it returns will return an empty set when queried with.
   *
   * @test_category paging
  ...
  public void should_fail_when_given_invalid_byte_array() {
    // Given an expected page state of size 1 and hash of size 1, we should expect 6 bytes, but only
    // receive 5.
