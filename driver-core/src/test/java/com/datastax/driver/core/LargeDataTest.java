...
public class LargeDataTest extends CCMTestsSupport {
  ...
  @CassandraVersion(
      value = "2.0.0",
      description = "< 2.0 is skipped as 1.2 does not handle reading wide rows well.")
  public void wideRows() throws Throwable {
    ...
    session()
        .execute(String.format("CREATE TABLE %s (k INT, i INT, PRIMARY KEY(k, i))", "wide_rows"));
    testWideRows(0);
  }
  ...
  @CassandraVersion(
      value = "2.0.0",
      description = "< 2.0 is skipped as 1.2 does not handle large batches well.")
  public void wideBatchRows() throws Throwable {
    session().execute(String.format(CREATE_KEYSPACE_SIMPLE_FORMAT, "large_data", 1));
    session().execute("USE large_data");
    session()
        .execute(
            String.format("CREATE TABLE %s (k INT, i INT, PRIMARY KEY(k, i))", "wide_batch_rows"));
    testWideBatchRows(0);
  }
  ...
  public void wideByteRows() throws Throwable {
    ...
    session()
        .execute(
            String.format("CREATE TABLE %s (k INT, i BLOB, PRIMARY KEY(k, i))", "wide_byte_rows"));
    testByteRows(0);
  }
