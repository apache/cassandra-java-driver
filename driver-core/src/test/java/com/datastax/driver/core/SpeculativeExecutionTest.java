...
public class SpeculativeExecutionTest {
  ...
  public void should_wait_until_all_executions_have_finished() {
    ...
    // The goal of this test is to check that execution2 does not fail the query, since execution1
    // is still running
    scassandras
        .node(2)
        .primingClient()
  ...
  /**
   * Validates that if a query gets speculatively re-executed, the second execution uses the same
   * default timestamp.
   *
  ...
  /**
   * Custom retry policy that retries at ONE on read timeout. This deals with the fact that
   * Scassandra only allows read timeouts with 0 replicas.
   */
