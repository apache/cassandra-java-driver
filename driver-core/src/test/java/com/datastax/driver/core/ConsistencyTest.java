...
public class ConsistencyTest {
   ...
   * ConsistencyLevel.SERIAL is the default serial consistency level, so even when sent it will
   * return as null.
   */
  ...
  /**
   * Tests that order of precedence is followed when defining CLs. Statement level CL should be
   * honored above QueryOptions. QueryOptions should be honored above default CL.
   *
   * @test_category consistency
   */
  @Test(groups = "short")
  public void should_use_appropriate_cl_when_multiple_defined() throws Throwable {
    ...
    // Build a cluster with no CL set in the query options.
    Cluster cluster =
        builder().withQueryOptions(new QueryOptions().setConsistencyLevel(cl_one)).build();
    try {
  ...
  @Test(
      groups = "short",
      dataProvider = "serialConsistencyLevels",
      dataProviderClass = DataProviders.class)
  public void should_use_query_option_serial_cl(ConsistencyLevel cl) throws Throwable {
    // Build a cluster with a CL level set in the query options.
    Cluster cluster =
        builder().withQueryOptions(new QueryOptions().setSerialConsistencyLevel(cl)).build();
    try {
      Session session = cluster.connect();
      // Construct unique query, with no CL defined.
