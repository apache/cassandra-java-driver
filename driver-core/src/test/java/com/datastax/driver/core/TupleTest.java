...
public class TupleTest extends CCMTestsSupport {
  ...
  /**
   * Basic test of tuple functionality. Original code found in
   * python-driver:integration.standard.test_types.py:test_tuple_type
   */
  @Test(groups = "short")
  ...
  /**
   * Test tuple types of lengths of 1, 2, 3, and 384 to ensure edge cases work as expected. Original
   * code found in python-driver:integration.standard.test_types.py:test_tuple_type_varying_lengths
   */
  @Test(groups = "short")
  public void tupleTestTypeVaryingLengths() throws Exception {
    ...
    session()
        .execute(
            String.format(
                "CREATE TABLE mytable (k int PRIMARY KEY, %s)", Joiner.on(',').join(valueSchema)));
    ...
    for (int i : lengths) {
      ...
      TupleType t =
          new TupleType(
              dataTypes, protocolVersion, cluster().getConfiguration().getCodecRegistry());
      TupleValue createdTuple = t.newValue(values.toArray());

      // write tuple
      session()
          .execute(String.format("INSERT INTO mytable (k, v_%s) VALUES (0, ?)", i), createdTuple);

      // read tuple
      TupleValue r =
          session()
              .execute(String.format("SELECT v_%s FROM mytable WHERE k=0", i))
              .one()
              .getTupleValue(String.format("v_%s", i));
      assertEquals(r, createdTuple);
    }
  }

  /**
   * Ensure tuple subtypes are appropriately handled. Original code found in
   * python-driver:integration.standard.test_types.py:test_tuple_subtypes
   */
  @Test(groups = "short")
  public void tupleSubtypesTest() throws Exception {
    ...
    session()
        .execute(
            String.format(
                ...
                Joiner.on(',').join(DATA_TYPE_PRIMITIVES)));
    ...
    for (DataType ignored : DATA_TYPE_PRIMITIVES) {
      ...
      TupleType t2 =
          new TupleType(
              completeDataTypes, protocolVersion, cluster().getConfiguration().getCodecRegistry());
      TupleValue createdTuple = t.newValue(createdValues.toArray());
      TupleValue completeTuple = t2.newValue(completeValues.toArray());

      // write tuple
      session()
          .execute(String.format("INSERT INTO mytable (k, v) VALUES (%s, ?)", i), createdTuple);
  ...
  /**
   * Ensure tuple subtypes are appropriately handled for maps, sets, and lists. Original code found
   * in python-driver:integration.standard.test_types.py:test_tuple_non_primitive_subtypes
   */
  @Test(groups = "short")
  public void tupleNonPrimitiveSubTypesTest() throws Exception {
    ...
    for (DataType datatype : DATA_TYPE_PRIMITIVES) {
      ...
      if (datatype != DataType.duration())
        values.add(
            String.format("v_%s frozen<tuple<map<%s, %s>>>", values.size(), datatype, datatype));
    }
    ...
    session()
        .execute(
            String.format(
                "CREATE TABLE mytable (k int PRIMARY KEY, %s)", Joiner.on(',').join(values)));
  ...
  private String nestedTuplesSchemaHelper(int depth) {
    ...
    else return String.format("frozen<tuple<%s>>", nestedTuplesSchemaHelper(depth - 1));
  }
  ...
  /**
   * Ensure nested are appropriately handled. Original code found in
   * python-driver:integration.standard.test_types.py:test_nested_tuples
   */
  @Test(groups = "short")
  public void nestedTuplesTest() throws Exception {
    ...
    for (int i : Arrays.asList(1, 2, 3, 32)) {
      ...
      session()
          .execute(
              String.format("INSERT INTO mytable (k, v_%s) VALUES (?, ?)", i), i, createdTuple);

      // verify tuple was written and read correctly
      TupleValue r =
          session()
              .execute(String.format("SELECT v_%s FROM mytable WHERE k=?", i), i)
              .one()
              .getTupleValue(String.format("v_%s", i));
  ...
  /**
   * Test for inserting null Tuple values into UDT's Original code found in
   * python-driver:integration.standard.test_types.py:test_tuples_with_nulls
   */
  @Test(groups = "short")
  public void testTuplesWithNulls() throws Exception {
    ...
    UserType userTypeDef =
        cluster().getMetadata().getKeyspace("testTuplesWithNulls").getUserType("user");
