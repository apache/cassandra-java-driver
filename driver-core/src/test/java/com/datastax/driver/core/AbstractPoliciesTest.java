...
public abstract class AbstractPoliciesTest extends CCMTestsSupport {
  ...
  protected void createMultiDCSchema(int dc1RF, int dc2RF) {
    ...
    session()
        .execute(
            String.format(
                CREATE_KEYSPACE_GENERIC_FORMAT,
                ks,
                "NetworkTopologyStrategy",
                String.format("'dc1' : %d, 'dc2' : %d", dc1RF, dc2RF)));
    useKeyspace(ks);
    session().execute(String.format("CREATE TABLE %s (k int PRIMARY KEY, i int)", tableName));
    check().before(5, MINUTES).that(new SchemaInAgreement(cluster())).becomesTrue();
  }
  ...
  protected void assertQueried(String host, int n) {
    try {
      ...
      else assertEquals(queried == null ? 0 : queried, n, queriedMapString());
    } catch (Exception e) {
  ...
  protected void init(int n, boolean batch, ConsistencyLevel cl) {
    write(n, batch, cl);
    prepared =
        session().prepare("SELECT * FROM " + tableName + " WHERE k = ?").setConsistencyLevel(cl);
  }
  ...
  protected void query(int n, boolean usePrepared, ConsistencyLevel cl) {
    if (usePrepared) {
      ...
      for (int i = 0; i < n; ++i) addCoordinator(session().execute(bs));
    } else {
      ...
      for (int i = 0; i < n; ++i)
        addCoordinator(
            session()
                .execute(
                    new SimpleStatement(String.format("SELECT * FROM %s WHERE k = 0", tableName))
