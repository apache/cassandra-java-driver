...
/**
 * A load balancing policy that sorts hosts on the last byte of the address, so that the query plan
 * is always [host1, host2, host3].
 */
public class SortingLoadBalancingPolicy implements LoadBalancingPolicy {

  final SortedSet<Host> hosts =
      new ConcurrentSkipListSet<Host>(
          new Comparator<Host>() {
            @Override
