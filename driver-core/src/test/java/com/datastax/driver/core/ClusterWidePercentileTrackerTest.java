...
public class ClusterWidePercentileTrackerTest
    extends PercentileTrackerTest<
        ClusterWidePercentileTracker.Builder, ClusterWidePercentileTracker> {
  ...
  public void should_track_all_measurements_for_cluster() {
    ...
    ClusterWidePercentileTracker tracker =
        builder().withInterval(1, TimeUnit.SECONDS).withMinRecordedValues(100).build();
    ...
    List<Exception> exceptions =
        Lists.newArrayList(
            new Exception(),
            null,
            new ReadTimeoutException(ConsistencyLevel.ANY, 1, 1, true),
            null,
            null);

    // when - recording latencies over a linear progression with varying hosts, statements and
    // exceptions.
    for (int i = 0; i < 100; i++) {
      tracker.update(
    ...
    // then - the resulting tracker's percentiles should represent that linear progression. (x
    // percentile == x)
    for (int i = 1; i <= 99; i++) {
