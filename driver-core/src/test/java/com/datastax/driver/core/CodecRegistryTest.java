...
public class CodecRegistryTest {
  ...
  public void should_find_codec_by_cql_type(DataType cqlType, TypeCodec<?> expected) {
    ...
    // then
    assertThat(actual).isNotNull().accepts(cqlType).isSameAs(expected);
  }

  @Test(groups = "unit", dataProvider = "cqlAndJava")
  public void should_find_codec_by_cql_type_java_type(
      DataType cqlType, Class<?> javaType, TypeCodec<?> expected) {
    // given
    CodecRegistry registry = new CodecRegistry();
    // when
    TypeCodec<?> actual = registry.codecFor(cqlType, javaType);
    // then
    assertThat(actual).isNotNull().accepts(cqlType).accepts(javaType).isSameAs(expected);
  }
  ...
  public void should_find_codec_by_value(Object value, TypeCodec<?> expected) {
    ...
    // then
    assertThat(actual).isNotNull().accepts(value).isSameAs(expected);
  }

  @Test(groups = "unit", dataProvider = "cqlAndValue")
  public void should_find_codec_by_cql_type_and_value(
      DataType cqlType, Object value, TypeCodec<?> expected) {
    // given
    CodecRegistry registry = new CodecRegistry();
    // when
    TypeCodec<?> actual = registry.codecFor(cqlType, value);
    // then
    assertThat(actual).isNotNull().accepts(cqlType).accepts(value).isSameAs(expected);
  }
  ...
  public void should_create_udt_codec() {
    CodecRegistry registry = new CodecRegistry();
    UserType udt =
        new UserType("ks", "test", false, Collections.<UserType.Field>emptyList(), V4, registry);
    assertThat(registry.codecFor(udt)).isNotNull().accepts(udt).accepts(UDTValue.class);
    registry = new CodecRegistry();
  ...
  public void should_ignore_codec_colliding_with_already_registered_codec() {
    ...
    assertThat(registry.codecFor(cint(), Integer.class)).isNotSameAs(newCodec);
  ...
  public void should_ignore_codec_colliding_with_already_generated_codec() {
    ...
    assertThat(logs.getNext()).contains("Ignoring codec MockCodec");
    assertThat(registry.codecFor(list(cint()), listOf(Integer.class))).isNotSameAs(newCodec);
