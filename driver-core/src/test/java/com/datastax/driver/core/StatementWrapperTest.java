...
public class StatementWrapperTest extends CCMTestsSupport {
  ...
  private static final String INSERT_MULTIPAGE_QUERY =
      "insert into test_multipage (k, v) values (?, ?)";
  ...
  CustomSpeculativeExecutionPolicy speculativeExecutionPolicy =
      new CustomSpeculativeExecutionPolicy();
  ...
  static class CustomLoadBalancingPolicy extends DelegatingLoadBalancingPolicy {
    ...
    public Iterator<Host> newQueryPlan(String loggedKeyspace, Statement statement) {
      if (statement instanceof CustomStatement) customStatementsHandled.incrementAndGet();
  ...
  /**
   * A retry policy that counts how many times it has seen the custom wrapper for UNAVAILABLE
   * errors.
   */
