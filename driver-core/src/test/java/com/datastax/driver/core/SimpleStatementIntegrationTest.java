...
public class SimpleStatementIntegrationTest extends CCMTestsSupport {
  ...
  public void should_fail_if_query_with_named_values_but_missing_parameter() {
    ...
    // Then - The driver does allow this because it doesn't know what parameters are required, but
    // C* should
  ...
  public void should_fail_if_query_with_named_values_but_using_wrong_type() {
    // Given a Statement using a named parameter with the wrong value for the type (id is of type
    // int, using double)
    SimpleStatement statement =
        new SimpleStatement(
            "SELECT * FROM users WHERE id = :id and id2 = :id2",
    ...
    // Then - The driver does allow this because it doesn't know the type information, but C* should
    // throw an
