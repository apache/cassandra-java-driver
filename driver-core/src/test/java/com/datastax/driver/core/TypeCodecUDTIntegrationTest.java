...
public class TypeCodecUDTIntegrationTest extends CCMTestsSupport {
  ...
  public void should_handle_udts_with_default_codecs() {
    ...
    session()
        .execute(ps.bind().setUUID(0, uuid).setString(1, "John Doe").setUDTValue(2, addressValue));
    rows = session().execute(selectQuery, uuid);
    row = rows.one();
    assertRow(row);
  }
  ...
  private void setUpUserTypes(Cluster cluster) {
    ...
    UDTValue phone1Value =
        phoneType.newValue().setString("number", phone1.number).setSet("tags", phone1.tags);
    UDTValue phone2Value =
        phoneType.newValue().setString("number", phone2.number).setSet("tags", phone2.tags);
    addressValue =
        addressType
            .newValue()
    ...
    protected Address deserialize(UDTValue value) {
      return value == null
          ...
          : new Address(
              ...
              value.getList("phones", Phone.class));
    }
    ...
    protected UDTValue serialize(Address value) {
      return value == null
          ...
          : userType
              ...
              .setInt("zipcode", value.zipcode)
              .setList("phones", value.phones, Phone.class);
    }
  }
  ...
  static class PhoneCodec extends MappingCodec<Phone, UDTValue> {
    ...
    protected Phone deserialize(UDTValue value) {
      return value == null
          ? null
          : new Phone(value.getString("number"), value.getSet("tags", String.class));
    }
    ...
    protected UDTValue serialize(Phone value) {
      return value == null
          ? null
          : userType.newValue().setString("number", value.number).setSet("tags", value.tags);
    }
  }
