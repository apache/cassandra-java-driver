...
public class GettableDataIntegrationTest extends CCMTestsSupport {
  ...
  public void onTestContextInitialized() {
    ...
    execute(
        "CREATE TABLE codec_mapping (k int PRIMARY KEY, "
  ...
  // Mappings of Codecs, Data Type to access as (used for determining what get|set method to call),
  // value to set.
  private TypeMapping<?>[] mappings = {
    ...
    // set|getDate
    new TypeMapping<LocalDate>(
        new IntToLocalDateCodec(), DataType.date(), LocalDate.fromDaysSinceEpoch(1523)),
    // set|getUUID
    new TypeMapping<UUID>(new IntToUUIDCodec(), DataType.uuid(), new UUID(244242, 0)),
    // set|getVarint
    new TypeMapping<BigInteger>(
        new IntToBigIntegerCodec(), DataType.varint(), BigInteger.valueOf(4566432L))
  };

  /**
   * Validates that all {@link GettableData} types will allow their get methods to be invoked on a
   * column that does not match data's cql type if a codec is registered that maps the java type of
   * the getter method to the cql type of the column.
   *
   * <p>Also validates that all {@link SettableData} types will allow their set methods to be
   * invoked on a column that does not match data's cql type if a codec is registered that maps the
   * java type of the setter method to the cql type of the column.
   *
   * <p>Executes the following for each set|get set:
   *
   * <p>
   *
   * <ol>
   *   <li>Insert row using a prepared statement and binding by name.
   *   <li>Insert row using a prepared statement and binding by index.
   *   <li>Insert row using a prepared statement and binding everything at once.
   *   <li>Retrieve inserted rows and get values by name.
   *   <li>Retrieve inserted rows and get values by index.
   * </ol>
   *
  ...
  public void should_allow_getting_and_setting_by_type_if_codec_registered() {
    String insertStmt =
        "INSERT INTO codec_mapping (k,v,l,m"
            + (is21 ? ",t,s" : "")
            + ") values (?,?,?,?"
            + (is21 ? ",?,?)" : ")");
    PreparedStatement insert = session().prepare(insertStmt);
    PreparedStatement select =
        session().prepare("SELECT v,l,m" + (is21 ? ",t,s" : "") + " from codec_mapping where k=?");
    ...
    for (TypeMapping<?> mapping : mappings) {
      ...
      for (int key : keys) {
        ...
        if (is21) {
          ...
          assertThat(getValue(returnedTuple, 0, mapping.outerType, registry))
              .isEqualTo(mapping.value);
          assertThat(getValue(returnedTuple, 1, mapping.outerType, registry))
              .isEqualTo(mapping.value);
