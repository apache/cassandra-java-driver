...
public class IndexMetadataTest extends CCMTestsSupport {
  ...
  private static final ColumnDefinitions legacyColumnDefs =
  ...
  private static final TypeCodec<Map<String, String>> MAP_CODEC =
      TypeCodec.map(TypeCodec.varchar(), TypeCodec.varchar());
  ...
  public void should_create_metadata_for_simple_index() {
    String createValuesIndex =
        String.format("CREATE INDEX text_column_index ON %s.indexing (text_column);", keyspace);
  ...
  @CassandraVersion(
      value = "2.1",
      description =
          "index names with quoted identifiers and collection indexes not supported until 2.1")
  public void should_create_metadata_for_values_index_on_mixed_case_column() {
    ...
    String createValuesIndex =
        ccm().getCassandraVersion().getMajor() > 2
            ? String.format(
                "CREATE INDEX \"MixedCaseIndex\" ON %s.indexing (values(\"MixedCaseColumn\"));",
                keyspace)
            : String.format(
                "CREATE INDEX \"MixedCaseIndex\" ON %s.indexing (\"MixedCaseColumn\");", keyspace);
    session().execute(createValuesIndex);
  ...
  public void should_create_metadata_for_index_on_map_values() {
    ...
    String createValuesIndex =
        ccm().getCassandraVersion().getMajor() > 2
            ? String.format(
                "CREATE INDEX map_values_index ON %s.indexing (values(map_values));", keyspace)
  ...
  public void should_create_metadata_for_index_on_map_keys() {
    String createKeysIndex =
        String.format("CREATE INDEX map_keys_index ON %s.indexing (keys(map_keys));", keyspace);
  ...
  public void should_create_metadata_for_full_index_on_map() {
    String createFullIndex =
        String.format("CREATE INDEX map_full_index ON %s.indexing (full(map_full));", keyspace);
  ...
  public void should_create_metadata_for_full_index_on_set() {
    String createFullIndex =
        String.format("CREATE INDEX set_full_index ON %s.indexing (full(set_full));", keyspace);
    session().execute(createFullIndex);
  ...
  public void should_create_metadata_for_full_index_on_list() {
    String createFullIndex =
        String.format("CREATE INDEX list_full_index ON %s.indexing (full(list_full));", keyspace);
    session().execute(createFullIndex);
  ...
  public void should_create_metadata_for_index_on_map_entries() {
    String createEntriesIndex =
        String.format(
            "CREATE INDEX map_entries_index ON %s.indexing (entries(map_entries));", keyspace);
  ...
  public void should_allow_multiple_indexes_on_map_column() {
    String createEntriesIndex =
        String.format(
            "CREATE INDEX map_all_entries_index ON %s.indexing (entries(map_all));", keyspace);
    session().execute(createEntriesIndex);
    String createKeysIndex =
        String.format("CREATE INDEX map_all_keys_index ON %s.indexing (keys(map_all));", keyspace);
    session().execute(createKeysIndex);
    String createValuesIndex =
        String.format(
            "CREATE INDEX map_all_values_index ON %s.indexing (values(map_all));", keyspace);
    session().execute(createValuesIndex);
  ...
  @Test(
      groups = "short",
      description =
          "This test case builds a ColumnMetadata object programmatically to test custom indices with pre-3.0 layout,"
  ...
  public void should_parse_legacy_custom_index_options() {
    ...
    Row columnRow =
        ArrayBackedRow.fromData(legacyColumnDefs, M3PToken.FACTORY, protocolVersion, columnData);
    ...
    assertThat(index)
        ...
        .asCqlQuery(
            String.format(
                "CREATE CUSTOM INDEX custom_index ON %s.indexing (text_column) "
                    + "USING 'dummy.DummyIndex' WITH OPTIONS = {'foo' : 'bar'};",
                keyspace));
  }
  ...
  @Test(
      groups = "short",
      description =
          "This test case builds a ColumnMetadata object programmatically to test custom indices with post-3.0 layout,"
              + "otherwise, it would require deploying an actual custom index class into the C* test cluster")
  public void should_parse_custom_index_options() {
    TableMetadata table = getTable("indexing");
    List<ByteBuffer> indexData =
        ImmutableList.of(
            wrap("custom_index"), // index name
            wrap("CUSTOM"), // kind
            MAP_CODEC.serialize(
                ImmutableMap.of(
                    "foo",
                    "bar",
                    IndexMetadata.CUSTOM_INDEX_OPTION_NAME,
                    "dummy.DummyIndex",
                    IndexMetadata.TARGET_OPTION_NAME,
                    "a, b, keys(c)"),
                protocolVersion) // options
            );
    Row indexRow =
        ArrayBackedRow.fromData(indexColumnDefs, M3PToken.FACTORY, protocolVersion, indexData);
    ...
    assertThat(index)
        ...
        .asCqlQuery(
            String.format(
                "CREATE CUSTOM INDEX custom_index ON %s.indexing (a, b, keys(c)) "
                    + "USING 'dummy.DummyIndex' WITH OPTIONS = {'foo' : 'bar'};",
                keyspace));
  }

  /**
   * Validates a special case where a 'KEYS' index was created using thrift. In this particular case
   * the index lacks index_options, however the index_options value is a 'null' string rather then a
   * null value.
   *
  ...
  public void should_parse_with_null_string_index_options() {
    ...
    assertThat(index)
        ...
        .asCqlQuery(
            String.format(
                "CREATE INDEX cfs_archive_parent_path ON %s.indexing (\"b@706172656e745f70617468\");",
                keyspace));
    assertThat(index.getOption(IndexMetadata.INDEX_KEYS_OPTION_NAME))
        .isNull(); // While the index type is KEYS, since it lacks index_options it does not get
    // considered.
  }
