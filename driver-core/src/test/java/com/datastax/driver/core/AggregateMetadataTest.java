...
public class AggregateMetadataTest extends CCMTestsSupport {
  ...
  public void should_parse_and_format_aggregate_with_initcond_and_no_finalfunc() {
    ...
    String cqlFunction =
        String.format(
            "CREATE FUNCTION %s.cat(s text,v int) RETURNS NULL ON NULL INPUT RETURNS text LANGUAGE java AS 'return s+v;';",
            keyspace);
    String cqlAggregate =
        String.format(
            "CREATE AGGREGATE %s.cat_tos(int) SFUNC cat STYPE text INITCOND '0';", keyspace);
    // when
    ...
    assertThat(aggregate.exportAsString())
        .isEqualTo(
            String.format(
                "CREATE AGGREGATE %s.cat_tos(int)\n"
                    + "    SFUNC cat\n"
                    + "    STYPE text\n"
                    + "    INITCOND '0';",
                this.keyspace));
  ...
  public void should_parse_and_format_aggregate_with_no_arguments() {
    ...
    String cqlFunction =
        String.format(
            "CREATE FUNCTION %s.inc(i int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return i+1;';",
            keyspace);
    String cqlAggregate =
        String.format("CREATE AGGREGATE %s.mycount() SFUNC inc STYPE int INITCOND 0;", keyspace);
    // when
    session().execute(cqlFunction);
    session().execute(cqlAggregate);
    // then
  ...
  public void should_parse_and_format_aggregate_with_final_function() {
    ...
    String cqlFunction1 =
        String.format(
            "CREATE FUNCTION %s.plus(i int, j int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return i+j;';",
            keyspace);
    String cqlFunction2 =
        String.format(
            "CREATE FUNCTION %s.announce(i int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return i;';",
            keyspace);
    String cqlAggregate =
        String.format(
            "CREATE AGGREGATE %s.prettysum(int) SFUNC plus STYPE int FINALFUNC announce INITCOND 0;",
            keyspace);
    // when
    ...
    assertThat(aggregate.exportAsString())
        .isEqualTo(
            String.format(
                "CREATE AGGREGATE %s.prettysum(int)\n"
                    + "    SFUNC plus\n"
                    + "    STYPE int\n"
                    + "    FINALFUNC announce\n"
                    + "    INITCOND 0;",
  ...
  public void should_parse_and_format_aggregate_with_no_initcond() {
    ...
    String cqlFunction =
        String.format(
            "CREATE FUNCTION %s.plus2(i int, j int) CALLED ON NULL INPUT RETURNS int LANGUAGE java AS 'return i+j;';",
            keyspace);
    String cqlAggregate =
        String.format("CREATE AGGREGATE %s.sum(int) SFUNC plus2 STYPE int;", keyspace);
    // when
    session().execute(cqlFunction);
    session().execute(cqlAggregate);
    // then
  ...
  /**
   * Validates aggregates with DynamicCompositeType state types and an initcond value that is a
   * literal, i.e.: 's@foo:i@32' can be appropriately parsed and generate a CQL string with the init
   * cond as a hex string, i.e.: 0x80730003666f6f00806900040000002000.
   *
  ...
  /**
   * Validates aggregates with DynamicCompositeType state types and an initcond value that is a hex
   * string representing the bytes for the type, i.e.: 0x80730003666f6f00806900040000002000' can be
   * appropriately parsed and generates an equivalent CQL string.
   *
   * @jira_ticket JAVA-1046
   * @test_category metadata
   * @since 3.0.1
  ...
  public void parse_and_format_aggregate_with_composite_type(
      String aggregateName, String initCond) {
    // given
    DataType custom =
        DataType.custom(
    ...
    String cqlFunction =
        String.format(
            "CREATE FUNCTION %s.%s_id(i %s) "
                ...
                + "LANGUAGE java "
                + "AS 'return i;';",
            keyspace, aggregateName, custom, custom);
    session().execute(cqlFunction);

    String cqlAggregate =
        String.format(
            "CREATE AGGREGATE %s.%s() " + "SFUNC %s_id " + "STYPE %s " + "INITCOND %s;",
            keyspace, aggregateName, aggregateName, custom, initCond);
    ...
    String expectedAggregate =
        String.format(
            "CREATE AGGREGATE %s.%s() "
                + "SFUNC %s_id "
                + "STYPE %s "
                + "INITCOND 0x80730003666f6f00806900040000002000;",
            keyspace, aggregateName, aggregateName, custom);
