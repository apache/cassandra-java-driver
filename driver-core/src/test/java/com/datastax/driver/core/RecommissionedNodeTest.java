 ...
 *
 * <p>This class tests scenarios where these nodes have been recommissioned to another cluster and
 * come back up. The driver must detect that they are not part of the cluster anymore, and ignore
 * them.
 */
public class RecommissionedNodeTest {
  ...
  public void should_ignore_recommissioned_node_on_reconnection_attempt() throws Exception {
    ...
    // node1 will be our "recommissioned" node, for now we just stop it so that it stays in the
    // peers table.
    mainCcm.stop(1);
  ...
  public void should_ignore_recommissioned_node_on_session_init() throws Exception {
    ...
    // Create a session. This will try to open a pool to node 1 and find out that the cluster name
    // doesn't match.
    mainCluster.connect();

    // Node 1 should now be DOWN with no reconnection attempt
    assertThat(mainCluster)
        .host(1)
  ...
  @Test(
      groups = "long",
      enabled = false,
      description = "Disabled because requires specific C* version, not essential")
  @CassandraVersion("3.0.0")
  public void should_ignore_node_that_does_not_support_protocol_version_on_session_init()
      throws Exception {
    // Simulate the bug before starting the cluster
    mainCcmBuilder = CCMBridge.builder().withNodes(2);
    mainCcm = CCMCache.get(mainCcmBuilder);
    ...
    // Create a session. This will try to open a pool to node 1 and find that it doesn't support
    // protocol version.
    mainCluster.connect();
  ...
  public void teardown() {
    if (mainCluster != null) mainCluster.close();
    if (mainCcmBuilder != null) CCMCache.remove(mainCcmBuilder);
    if (otherCcmBuilder != null) CCMCache.remove(otherCcmBuilder);
    if (mainCcm != null) mainCcm.close();
    if (otherCcm != null) otherCcm.close();
  }
  ...
  private static void waitForCountUpHosts(Cluster cluster, int expectedCount)
    ...
    for (int i = 0; i <= maxRetries; i++) {
      ...
      if (i == maxRetries)
        fail(
            String.format(
                "Up host count didn't reach %d after %d seconds", expectedCount, i * interval));
      else logger.debug("Counted {} up hosts after {} seconds", actualCount, i * interval);
