...
public class RefreshConnectedHostTest extends CCMTestsSupport {
  ...
  public void should_refresh_single_connected_host() {
    ...
    LimitingLoadBalancingPolicy loadBalancingPolicy =
        new LimitingLoadBalancingPolicy(new RoundRobinPolicy(), 2, 1);
    ...
    // Wait for the node to be up, because apparently on Jenkins it's still only ADDED when we reach
    // this line
    // Waiting for NEW_NODE_DELAY_SECONDS+1 allows the driver to create a connection pool and mark
    // the node up
    assertThat(cluster)
        .host(2)
