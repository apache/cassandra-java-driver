 ...
 *
 * <p>There is no purging mechanism, so make sure it doesn't stay enabled for too long (this is best
...
public class MemoryAppender extends WriterAppender {
  ...
  /**
   * Wait until at least one log event is appended to the current appender, or a timeout occurs,
   * whichever happens first, then return the appender contents. Not thread safe. Useful when
   * asynchronous code needs to wait until the appender is actually invoked at least once.
   *
