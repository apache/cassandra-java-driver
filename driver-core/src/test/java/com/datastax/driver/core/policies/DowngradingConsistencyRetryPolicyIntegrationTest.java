...
public class DowngradingConsistencyRetryPolicyIntegrationTest
  ...
  /**
   * @return An array of pairs that match # of alive replicas with the expected downgraded CL used
   *     on read/write/unavailable.
   */
  @DataProvider
  ...
  /**
   * Ensures that when handling a read timeout with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is reattempted with {@link ConsistencyLevel#ONE} if the consistency level on the
   * statement executed is {@link ConsistencyLevel#EACH_QUORUM}, even if the number of known alive
   * replicas was 0.
   *
  ...
  /**
   * Ensures that when handling a read timeout with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is reattempted with a {@link ConsistencyLevel} that matches min(received
   * acknowledgements, THREE) and is only retried once.
   *
  ...
  public void should_retry_once_on_same_host_with_reduced_consistency_level_on_read_timeout(
      int received, ConsistencyLevel expectedDowngradedCL) {
  ...
  /**
   * Ensures that when handling a read timeout with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is reattempted if data was not retrieved, but enough replicas were alive to handle the
   * request.
   *
   * @test_category retry_policy
   */
  ...
  /**
   * Ensures that when handling a read timeout with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is not attempted if no replicas were alive. In a real scenario, this would not be
   * expected as we'd anticipate an {@link UnavailableException} instead.
   *
   * @test_category retry_policy
   */
  @Test(groups = "short")
  ...
  /**
   * Ensures that when handling a write timeout with {@link DowngradingConsistencyRetryPolicy} that
   * it rethrows the exception if the {@link WriteType} was any of {@link #rethrowWriteTypes}.
   *
  ...
  /**
   * Ensures that when handling a write timeout with {@link DowngradingConsistencyRetryPolicy} that
   * it ignores the exception if the {@link WriteType} was any of {@link
   * #ignoreWriteTypesWithReceivedAcks} and we received acks some at least one replica.
   *
   * @param writeType writeType communicated by {@link WriteTimeoutException}.
   * @test_category retry_policy
   */
  ...
  /**
   * Ensures that when handling a write timeout with {@link DowngradingConsistencyRetryPolicy} that
   * a retry is attempted on the same host if the {@link WriteType} is {@link WriteType#BATCH_LOG}.
   *
   * @test_category retry_policy
   */
  @Test(groups = "short")
  ...
  /**
   * Ensures that when handling a write timeout with {@link DowngradingConsistencyRetryPolicy} that
   * a retry is attempted on the same host with a reduced consistency level that matches
   * min(received acknowledgments, THREE) if the {@link WriteType} is {@link
   * WriteType#UNLOGGED_BATCH} and is only retries once.
   *
   ...
   */
  @Test(groups = "short", dataProvider = "consistencyLevels")
  public void should_retry_once_on_same_host_with_reduced_consistency_level_on_write_timeout(
      int alive, ConsistencyLevel expectedDowngradedCL) {
    simulateError(
        1, write_request_timeout, new WriteTimeoutConfig(UNLOGGED_BATCH, alive, alive + 1));
  ...
  /**
   * Ensures that when handling an unavailable with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is reattempted with a {@link ConsistencyLevel} that matches min(received
   * acknowledgements, THREE) and is only retried once.
   *
   ...
   */
  @Test(groups = "short", dataProvider = "consistencyLevels")
  public void should_retry_once_on_same_host_with_reduced_consistency_level_on_unavailable(
      int alive, ConsistencyLevel expectedDowngradedCL) {
  ...
  /**
   * Ensures that when handling an unavailable with {@link DowngradingConsistencyRetryPolicy} that a
   * retry is is not reattempted if no replicas are alive.
   *
   * @test_category retry_policy
   */
  @Test(groups = "short")
