...
import static org.scassandra.http.client.Result.invalid;
...
import static org.scassandra.http.client.Result.syntax_error;
...
public class ErrorAwarePolicyIntegrationTest {
  ...
  public void setUp() {
    ...
    latencyTracker =
        new LatencyTracker() {
          @Override
          public void update(
              Host host, Statement statement, Exception exception, long newLatencyNanos) {
            if (exception != null) errorCounter.incrementAndGet();
          }
  ...
  /**
   * Checks that {@link LatencyTracker#update} was called at least expectedCount times within 5
   * seconds.
   *
   * <p>Note that the usefulness of this is dependent on the {@link
   * ErrorAwarePolicy.PerHostErrorTracker} being invoked before the latency tracker being used to
   * track error invocations in this test. The existing implementation seems to invoke update on
   * latency trackers in order that they are registered.
   *
  ...
  /**
   * Validates that {@link ErrorAwarePolicy} properly excludes a host after the maximum number of
   * errors is exceeded.
   *
   * <p>This test configures a maximum of 1 error per minute and executes 2 failing queries against
   * host1 during the first 5 simulated seconds. host1 should be excluded as soon as the rolling
   * count updates over the next 5-second interval.
   *
   * <p>It then makes another query and ensures it is executed against host2 and that the response
   * was successful.
   *
  ...
  /**
   * Validates that {@link ErrorAwarePolicy} will include a previously excluded host after the
   * configured retry period has elapsed.
   *
   * <p>The test executes queries with error to get host1 excluded. It then executes queries over 70
   * simulated seconds and then executes another query after this time has elapsed and ensures that
   * the next query execution uses host1.
   *
   * @jira_ticket JAVA-1055
   * @test_category load_balancing:error_aware
   * @since 3.1.0
  ...
  /**
   * Validates that {@link ErrorAwarePolicy} will not penalize errors that are not considered in the
   * default {@link ErrorAwarePolicy.ErrorFilter} implementation.
   *
   * <p>Executes 10 queries with each error type and ensures that host1 is used each time, verifying
   * that it was never excluded.
   *
   * @jira_ticket JAVA-1055
   * @test_category load_balancing:error_aware
   * @since 3.1.0
  ...
  /**
   * Validates that {@link ErrorAwarePolicy} will regard a custom {@link
   * ErrorAwarePolicy.ErrorFilter} by only penalizing a node when it produces exceptions that
   * evaluate to true in the filter implementation.
   *
   * <p>It first executes 10 queries with an error type that is not considered and verify that host1
   * is never excluded.
   *
   * <p>It then executes queries with an error type that is considered and verifies that host1 is
   * then excluded and host2 is used instead.
   *
   * @jira_ticket JAVA-1055
   * @test_category load_balancing:error_aware
   * @since 3.1.0
  ...
  public void should_only_consider_exceptions_based_on_errors_filter() throws InterruptedException {
    ErrorAwarePolicy.ErrorFilter iqeOnlyFilter =
        new ErrorAwarePolicy.ErrorFilter() {
          @Override
    ...
    try {
      ...
      // The first errors that were considered were at t = 5 seconds and this is when the rolling
      // count was
  ...
  /**
   * Validates that an {@link ErrorAwarePolicy} configured with its defaults behaves as documented,
   * that being that the maximum number of errors is 1 and the retry period is 120 seconds.
   *
   * @jira_ticket JAVA-1055
   * @test_category load_balancing:error_aware
   * @since 3.1.0
  ...
  public void should_regard_defaults() throws InterruptedException {
    ...
    try {
      ...
      // Advance clock 30 seconds, this is within the retry period so host 1 should still be
      // excluded.
      setTime(35, SECONDS);
      queryTracker.query(session, 5, sCluster.address(2));

      // At this point 120 seconds have elapsed, the load balancing policy should see that the retry
      // period has
