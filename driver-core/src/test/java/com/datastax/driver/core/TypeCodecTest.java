...
public class TypeCodecTest {
  ...
  public void test_varchar_vs_text() {
    ...
    UserType u1 =
        new UserType(
            ...
            "table",
            false,
            newArrayList(new Field("f1", varchar()), new Field("f2", varchar())),
            V3,
            new CodecRegistry());
    UserType u2 =
        new UserType(
            ...
            "table",
            false,
            newArrayList(new Field("f1", text()), new Field("f2", varchar())),
            V3,
            new CodecRegistry());
    UserType u3 =
        new UserType(
            ...
            "table",
            false,
            newArrayList(new Field("f1", varchar()), new Field("f2", text())),
            V3,
            new CodecRegistry());
    UserType u4 =
        new UserType(
            ...
            "table",
            false,
            newArrayList(new Field("f1", text()), new Field("f2", text())),
            V3,
            new CodecRegistry());
    assertThat(TypeCodec.userType(u1)).accepts(u2).accepts(u3).accepts(u4);
  }
  ...
  public void should_deserialize_empty_buffer_as_tuple_with_null_values() {
    ...
    TupleType tupleType =
        new TupleType(
            newArrayList(DataType.cint(), DataType.varchar(), DataType.cfloat()),
    ...
    TupleValue actual =
        codecRegistry
            .codecFor(tupleType, TupleValue.class)
            .deserialize(ByteBuffer.allocate(0), ProtocolVersion.NEWEST_SUPPORTED);
  ...
  /**
   * Ensures that {@link TypeCodec#timeUUID()} is resolved for all UUIDs and throws an {@link
   * InvalidTypeException} when attempting to serialize or format a non-type 1 UUID.
   *
  ...
  private class ListVarcharToListListInteger extends TypeCodec<List<List<Integer>>> {
    ...
    public List<List<Integer>> deserialize(ByteBuffer bytes, ProtocolVersion protocolVersion) {
      return Lists.transform(
          codec.deserialize(bytes, protocolVersion),
  ...
  class ACodec extends TypeCodec<A> {
    ...
    @Override
    public ByteBuffer serialize(A value, ProtocolVersion protocolVersion)
        throws InvalidTypeException {
      return null; // not tested
    }

    @Override
    public A deserialize(ByteBuffer bytes, ProtocolVersion protocolVersion)
        throws InvalidTypeException {
      return null; // not tested
    }
  ...
  class BCodec extends TypeCodec<B> {
    ...
    @Override
    public ByteBuffer serialize(B value, ProtocolVersion protocolVersion)
        throws InvalidTypeException {
      return null; // not tested
    }

    @Override
    public B deserialize(ByteBuffer bytes, ProtocolVersion protocolVersion)
        throws InvalidTypeException {
      return null; // not tested
    }
