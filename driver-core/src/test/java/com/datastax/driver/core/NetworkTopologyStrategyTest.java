...
public class NetworkTopologyStrategyTest extends AbstractReplicationStrategyTest {
  ...
  private static ReplicationStrategy networkTopologyStrategy(ReplicationFactorDefinition... rfs) {
    Builder<String, String> builder =
        ImmutableMap.<String, String>builder().put("class", "NetworkTopologyStrategy");
  ...
  private static final ReplicationStrategy exampleStrategy =
      networkTopologyStrategy(rf(DC1, 2), rf(DC2, 2));

  private static final ReplicationStrategy exampleStrategyTooManyReplicas =
      networkTopologyStrategy(rf(DC1, 4), rf(DC2, 4));
  ...
  private static final List<Token> exampleRing =
  ...
  private static final Map<Token, Host> exampleTokenToPrimary =
      ImmutableMap.<Token, Host>builder()
  ...
  public void networkTopologyWithSimpleDCLayoutTest1() {
    List<Token> ring =
        ImmutableList.<Token>builder().add(TOKEN01).add(TOKEN04).add(TOKEN14).add(TOKEN19).build();
  ...
  public void should_warn_if_replication_factor_cannot_be_met() {
    ...
    try {
      ...
      assertThat(logs.getNext())
          .contains(
              String.format(
                  "Error while computing token map for keyspace %s with datacenter %s",
                  keyspace, DC2));
