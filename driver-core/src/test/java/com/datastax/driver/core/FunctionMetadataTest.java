...
public class FunctionMetadataTest extends CCMTestsSupport {
  ...
  public void should_parse_and_format_simple_function() {
    ...
    String cql =
        String.format(
            "CREATE FUNCTION %s.plus(s int,v int) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return s+v;';",
            keyspace);
    // when
    session().execute(cql);
    // then
    ...
    assertThat(function.exportAsString())
        .isEqualTo(
            String.format(
                "CREATE FUNCTION %s.plus(s int,v int)\n"
  ...
  public void should_parse_and_format_function_with_no_arguments() {
    ...
    String cql =
        String.format(
            "CREATE FUNCTION %s.pi() CALLED ON NULL INPUT RETURNS double LANGUAGE java AS 'return Math.PI;';",
            keyspace);
    // when
    session().execute(cql);
    // then
  ...
  /**
   * Ensures that functions whose arguments contain complex types such as tuples and collections,
   * and nested combinations thereof, are correctly parsed.
   *
  ...
  public void should_parse_and_format_functions_with_complex_arguments() {
    ...
    String cql =
        String.format(
            "CREATE FUNCTION %s.complex(x tuple<tuple<int>, map<int, int>>) RETURNS NULL ON NULL INPUT RETURNS int LANGUAGE java AS 'return 42;';",
            keyspace);
    // when
    session().execute(cql);
    // then
    ...
    assertThat(function.exportAsString())
        .isEqualTo(
            String.format(
                "CREATE FUNCTION %s.complex(x tuple<tuple<int>, map<int, int>>)\n"
                    + "    RETURNS NULL ON NULL INPUT\n"
                    + "    RETURNS int\n"
                    + "    LANGUAGE java\n"
                    + "    AS 'return 42;';",
