...
public class HostMetadataIntegrationTest {
  ...
  /**
   * Validates that {@link Host#getDseVersion()} and {@link Host#getDseWorkload()} return values
   * defined in the <code>dse_version</code> and <code>workload</code> columns if they are present
   * in <code>system.local</code> or <code>system.peers</code> otherwise they return null.
   *
  ...
  public void should_parse_dse_workload_and_version_if_available() {
    ...
    try {
      ...
      assertThat(cluster)
          .host(1)
          .hasWorkload("Analytics")
          .hasDseVersion(VersionNumber.parse("4.8.4"));
      ...
      assertThat(cluster)
          .host(4)
          .hasWorkload("Cassandra")
          .hasDseVersion(VersionNumber.parse("4.8.4"));
      assertThat(cluster)
          .host(5)
          .hasWorkload("AmazingNewFeature")
          .hasDseVersion(VersionNumber.parse("5.0.0"));
    } finally {
      cluster.close();
   ...
   * the <code>dse_version</code> and <code>workload</code> columns are not present in <code>
   * system.local</code> for the control host.
   *
   * @test_category host:metadata
   * @jira_ticket JAVA-1042
   */
  ...
  public void should_not_parse_dse_workload_and_version_if_not_present_in_local_table() {
    // given: A cluster with node 1 (control host) not having a workload or dse_version set, and
    // node 2 with those
    //        columns set.
    ScassandraCluster scassandraCluster =
        ScassandraCluster.builder()
  ...
  /**
   * Validates that if <code>dse_version</code> column is a non-version value in <code>system.local
   * </code> or <code>system.peers</code> that a warning is logged and {@link Host#getDseVersion()}
   * returns null.
   *
   * @test_category host:metadata
   * @jira_ticket JAVA-1042
  ...
  /**
   * Validates that {@link Host#isDseGraphEnabled()} returns the value defined in the <code>graph
   * </code> columns if it is present in <code>system.local</code> or <code>system.peers</code>
  ...
  /**
   * Validates that {@link Host#isDseGraphEnabled()} returns false if the <code>graph</code> column
   * is not present in <code>system.local</code> for the control host.
   *
   * @test_category host:metadata
   * @jira_ticket JAVA-1171
   */
  ...
  /**
   * Validates that {@link Host#getBroadcastAddress()} is set for all hosts (control host or not) in
   * the default case of a cluster, as the broadcast address is derived from <code>broadcast_address
   * </code> in <code>system.local</code> and <code>peer</code> in <code>system.peers</code>.
   *
   * @test_category host:metadata
  ...
  public void should_set_broadcast_address_for_all_nodes() {
    ...
    ScassandraCluster scassandraCluster =
        ScassandraCluster.builder().withIpPrefix(TestUtils.IP_PREFIX).withNodes(3).build();
    ...
    try {
      ...
      //  - broadcast_address is used for system.local and is always present, so control host should
      // have it.
  ...
  /**
   * Validates that {@link Host#getListenAddress()} is set for a host if and only if the <code>
   * listen_address</code> column is present and set in the <code>system.peers</code> table or if
   * the host is the control host.
   *
   * @test_category host:metadata
   * @jira_ticket JAVA-1035
   */
  ...
  public void should_set_listen_address_if_available() {
    // given: A Cluster with 3 nodes and node 2 being configured with a listen address in its peers
    // table.
