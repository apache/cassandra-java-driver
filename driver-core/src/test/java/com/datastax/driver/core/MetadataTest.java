...
public class MetadataTest extends CCMTestsSupport {
  ...
  /**
   * Validates that when the topology of the cluster changes that Cluster Metadata is properly
   * updated.
   *
   * <p>
   *
   ...
   *
   * <ol>
   *   <li>Creates a 3 node cluster and capture token range data from the {@link Cluster}'s {@link
   *       Metadata}
   *   <li>Decommission node 3.
   *   <li>Validates that the token range data was updated to reflect node 3 leaving and the other
   *       nodes taking on its token range.
   *   <li>Adds a new node, node 4.
   *   <li>Validates that the token range data was updated to reflect node 4 joining the {@link
   *       Cluster} and the token ranges reflecting this.
   * </ol>
   *
   * @test_category metadata:token
   * @expected_result cluster metadata is properly updated in response to node remove and add
   *     events.
