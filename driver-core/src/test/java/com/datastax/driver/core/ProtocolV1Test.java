...
public class ProtocolV1Test extends CCMTestsSupport {
  ...
  public Cluster.Builder createClusterBuilder() {
    return super.createClusterBuilder().withProtocolVersion(V1);
  }
  ...
  /**
   * Validates that a simple query with variables is not allowed with protocol V1. (Values in
   * protocol V1 are only allowed in prepared statements).
   *
   * @jira_ticket JAVA-1132
   */
  @Test(groups = "short")
    ...
    try {
      ...
      assertThat(e)
          .hasMessageContaining(
              "Unsupported feature with the native protocol V1 (which is currently in use): Binary values are not supported");
    }
  }
