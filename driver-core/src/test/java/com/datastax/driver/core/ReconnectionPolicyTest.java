...
public class ReconnectionPolicyTest extends AbstractPoliciesTest {
  ...
  private Cluster.Builder exponential() {
    return Cluster.builder()
        .withReconnectionPolicy(new ExponentialReconnectionPolicy(2 * 1000, 5 * 60 * 1000));
  }
  ...
  public void exponentialReconnectionPolicyTest() throws Throwable {
    ...
    ReconnectionPolicy.ReconnectionSchedule schedule =
        new ExponentialReconnectionPolicy(2 * 1000, 5 * 60 * 1000).newSchedule();
  ...
  public void constantReconnectionPolicyTest() throws Throwable {
    ...
    // Test nextDelays()
    ReconnectionPolicy.ReconnectionSchedule schedule =
        new ConstantReconnectionPolicy(10 * 1000).newSchedule();
  ...
  public void reconnectionPolicyTest(long restartTime, long retryTime, long breakTime)
      throws Throwable {
    createSchema(1);
    init(12);
    query(12);
    ...
    while (true) {
      ...
      try {
        ...
        assertTrue(
            retryTime - 2 < thisTime - startTime && thisTime - startTime < retryTime + 2,
            String.format(
                "Waited %s seconds instead an expected %s seconds wait",
                thisTime - startTime, retryTime));
      } catch (NoHostAvailableException e) {
        Thread.sleep(1000);
        continue;
      }
