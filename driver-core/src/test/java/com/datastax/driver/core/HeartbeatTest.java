...
public class HeartbeatTest extends ScassandraTestBase {
  ...
  /**
   * Ensures that a heartbeat message is sent after the configured heartbeat interval of idle time
   * and succeeds and continues to be sent as long as the connection remains idle.
   *
  ...
  /**
   * Ensures that a heartbeat message is sent after the configured heartbeat interval of idle time
   * when no data is received on a connection even though are successful writes on the socket.
   *
   * @test_category connection:heartbeat
   * @expected_result heartbeat is sent after heartbeat interval (3) seconds of idle time.
  ...
  public void should_send_heartbeat_when_requests_being_written_but_nothing_received()
    ...
    try {
      ...
      // Define patterns that check for whether or not heartbeats are sent / received on a given
      // connection.
      Pattern heartbeatSentPattern = Pattern.compile(".*" + connectionName + ".*sending heartbeat");
      Pattern heartbeatReceivedPattern =
          Pattern.compile(".*" + connectionName + ".*heartbeat query succeeded");
      ...
      // Ensure heartbeat is sent after no received data, even though we have inflight requests
      // (JAVA-1346).
      SECONDS.sleep(4);
   ...
   *
   * <p>While difficult to prove the absence of evidence, the test will wait up to the default
   * heartbeat interval (30 seconds + 1) and check to see if the heartbeat was sent.
   *
   * @test_category connection:heartbeat
   * @expected_result heartbeat is not sent after default heartbeat interval (60) seconds of idle
   *     time.
   * @jira_ticket JAVA-533
   * @since 2.0.10, 2.1.5
   */
