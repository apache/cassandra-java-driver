...
public class AlterTest {
  ...
  public void should_alter_column_type_to_UDT() throws Exception {
    ...
    SchemaStatement statement =
        alterTable("ks", "test").alterColumn("address").udtType(frozen("address"));
  ...
  public void should_alter_table_options() throws Exception {
    // When
    // Note that this does not necessarily represent a valid configuration, the goal is just to test
    // all options
    ...
    SchemaStatement statementWith21Caching =
    ...
    assertThat(statementWith21Caching.getQueryString())
        .isEqualTo(
            "\n\tALTER TABLE test\n\t"
  ...
  public void should_fail_if_keyspace_name_is_a_reserved_keyword() throws Exception {
    alterTable("add", "test").addColumn("test").type(DataType.ascii());
  }
  ...
  public void should_fail_if_table_name_is_a_reserved_keyword() throws Exception {
    alterTable("add").addColumn("test").type(DataType.ascii());
  }
  ...
  @Test(
      ...
      expectedExceptions = IllegalArgumentException.class,
      expectedExceptionsMessageRegExp =
          "The renamed column name 'add' is not allowed because it is a reserved keyword")
  public void should_fail_if_renamed_column_is_a_reserved_keyword() throws Exception {
    alterTable("test").renameColumn("add");
  }
