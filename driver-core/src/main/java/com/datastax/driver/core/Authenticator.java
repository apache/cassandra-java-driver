 ...
 *
 * <ol>
 *   <li>The {@code initialResponse} method will be called. The initial return value will be sent to
 *       the server to initiate the handshake.
 ...
 *       the authenticator {@code evaluateChallenge} method will be called to produce a response
 *       that will be sent to the server. This challenge/response negotiation will continue until
 *       the server responds that authentication is successful (or an {@code
 *       AuthenticationException} is raised).
 *   <li>When the server indicates that authentication is successful, the {@code
...
public interface Authenticator {
   ...
   * @return updated SASL token, may be null to indicate the client requires no further action
   */
