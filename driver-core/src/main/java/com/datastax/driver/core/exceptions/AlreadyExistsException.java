...
public class AlreadyExistsException extends QueryValidationException
  ...
  private static String makeMsg(String keyspace, String table) {
    if (table.isEmpty()) return String.format("Keyspace %s already exists", keyspace);
   ...
   *
   * @return {@code true} if this exception is raised following a table creation attempt, {@code
   *     false} if it was a keyspace creation attempt.
   */
   ...
   *
   * @return a keyspace name that is either the keyspace whose creation attempt failed because a
   *     keyspace of the same name already exists (in that case, {@link #table} will return {@code
   *     null}), or the keyspace of the table creation attempt (in which case {@link #table} will
   *     return the name of said table).
   */
   ...
   *
   * @return the name of table whose creation attempt failed because a table of this name already
   *     exists, or {@code null} if the query was a keyspace creation query.
   */
