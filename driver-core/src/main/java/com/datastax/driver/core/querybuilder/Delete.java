...
public class Delete extends BuiltStatement {
   ...
   *
   * <p>This is a shorter/more readable version for {@code onlyIf().and(condition)}.
   *
   ...
   *
   * <p>Chain this with {@link Options#and(Using)} to add options.
   *
   * @return the options of this DELETE statement.
   */
   ...
   * <p>
   *
   * <p>A delete with that option will report whether the statement actually resulted in data being
   * deleted. The existence check and deletion are done transactionally in the sense that if
   * multiple clients attempt to delete a given row with this option, then at most one may succeed.
   *
   * <p>Please keep in mind that using this option has a non negligible performance impact and
   * should be avoided when possible. This will configure the statement as non-idempotent, see
  ...
  public static class Where extends BuiltStatement.ForwardingStatement<Delete> {
     ...
     * <p>
     *
     * <p>A delete with that option will report whether the statement actually resulted in data
     * being deleted. The existence check and deletion are done transactionally in the sense that if
     * multiple clients attempt to delete a given row with this option, then at most one may
     * succeed.
     *
     * <p>Please keep in mind that using this option has a non negligible performance impact and
    ...
    public Builder all() {
      if (!columns.isEmpty())
        throw new IllegalStateException(
            String.format("Some columns (%s) have already been selected.", columns));
  ...
  /**
   * A selector in a DELETE selection clause. A selector can be either a column name, a list
   * element, a set element or a map entry.
   */
   ...
   *
   * <p>When provided some conditions, a deletion will not apply unless the provided conditions
   * applies.
   *
   * <p>Please keep in mind that provided conditions have a non negligible performance impact and
   * should be avoided when possible.
   */
  public static class Conditions extends BuiltStatement.ForwardingStatement<Delete> {
     ...
     * Cassandra currently only allows equality ones.
     *
     * @param condition the condition to add.
