 ...
 *
 * <p>A {@code columnDefinitions}} instance is mainly a list of {@code
 * ColumnsDefinitions.Definition}. The definitions or metadata for a column can be accessed either
 * by:
 *
 * <ul>
 *   <li>index (indexed from 0)
 *   <li>name
 * </ul>
 *
 * <p>When accessed by name, column selection is case insensitive. In case multiple columns only
 * differ by the case of their name, then the column returned with be the first column that has been
 ...
 *
 * <ul>
 *   <li>If {@code cd} contains column {@code fOO}, then {@code cd.contains("foo")}, {@code
 *       cd.contains("fOO")} and {@code cd.contains("Foo")} will return {@code true}.
 *   <li>If {@code cd} contains both {@code foo} and {@code FOO} then:
 *       <ul>
 *         <li>{@code cd.getType("foo")}, {@code cd.getType("fOO")} and {@code cd.getType("FOO")}
 *             will all match column {@code foo}.
 *         <li>{@code cd.getType("\"FOO\"")} will match column {@code FOO}
 *       </ul>
 * </ul>
 *
...
public class ColumnDefinitions implements Iterable<ColumnDefinitions.Definition> {
  ...
  static final ColumnDefinitions EMPTY =
      new ColumnDefinitions(new Definition[0], CodecRegistry.DEFAULT_INSTANCE);
   ...
   * @return the index of the first occurrence of {@code name} in this metadata if {@code
   *     contains(name)}, -1 otherwise.
   */
   ...
   *
   * <p>The order of the iterator will be the one of this metadata.
   *
   ...
   *
   * <p>Note that this method does not set the {@link DataType#isFrozen()} flag on the returned
   * object, it will always default to {@code false}. Use {@link Cluster#getMetadata()} to determine
   * if a column is frozen.
   *
   * @param i the index in this metadata.
  ...
  int[] findAllIdx(String name) {
    ...
    for (int i = 0; i < indexes.length; i++) {
      int idx = indexes[i];
      if (name.equals(byIdx[idx].name)) result[j++] = idx;
    }
