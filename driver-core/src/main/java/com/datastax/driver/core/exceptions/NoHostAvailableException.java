 ...
 *
 * <p>This exception is thrown if either:
 *
 * <ul>
 *   <li>there is no host live in the cluster at the moment of the query;
 *   <li>all hosts that have been tried have failed.
 * </ul>
 *
 * <p>For debugging purposes, the list of hosts that have been tried along with the failure cause
 * can be retrieved using the {@link #getErrors()} method.
 */
public class NoHostAvailableException extends DriverException {
   ...
   *
   * @param maxErrors the maximum number of errors displayed (useful to limit the size of the
   *     message for big clusters). Beyond this limit, host names are still displayed, but not the
   *     associated errors. Set to {@code Integer.MAX_VALUE} to display all hosts.
   * @param formatted whether to format the output (line break between each host).
   * @param includeStackTraces whether to include the full stacktrace of each host error. Note that
   *     this automatically implies {@code formatted}.
   * @return the message.
   */
  public String getCustomMessage(int maxErrors, boolean formatted, boolean includeStackTraces) {
    if (includeStackTraces) formatted = true;
  ...
  private static String makeMessage(
    ...
    if (errors.size() == 0) return "All host(s) tried for query failed (no host was tried)";
    ...
    if (truncated) {
      out.print(formatted ? "\n" : " ");
      out.printf(
          "[only showing errors of first %d hosts, use getErrors() for more details]",
          maxErrorsInMessage);
    }
    if (formatted && !includeStackTraces) out.print("\n");
    out.print(")");
    out.close();
