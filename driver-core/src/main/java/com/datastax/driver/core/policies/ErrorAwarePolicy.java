 ...
 *
 * <p>When creating a query plan, this policy gathers a list of candidate hosts from its child
 * policy; for each candidate host, it then determines whether it should be included into or
 * excluded from the final query plan, based on its current error rate (measured over the last
 * minute, with a 5-second granularity).
 *
 * <p>Note that the policy should not blindly count all errors in its measurements: some type of
 * errors (e.g. CQL syntax errors) can originate from the client and occur on all hosts, therefore
 * they should not count towards the exclusion threshold or all hosts could become excluded. You can
 * provide your own {@link ErrorFilter} to customize that logic.
 *
 * <p>The policy follows the builder pattern to be created, the {@link Builder} class can be created
 * with {@link #builder} method.
 *
 * <p>This policy is currently in BETA mode and its behavior might be changing throughout different
 * driver versions.
 */
@Beta
public class ErrorAwarePolicy implements ChainableLoadBalancingPolicy {
  ...
  private ErrorAwarePolicy(Builder builder) {
    ...
    this.errorTracker =
        new PerHostErrorTracker(builder.maxErrorsPerMinute, builder.errorFilter, builder.clock);
  }
  ...
  public static class Builder {
     ...
     *
     * <p>The policy keeps track of the number of errors on each host (filtered by {@link
     * Builder#withErrorsFilter(com.datastax.driver.core.policies.ErrorAwarePolicy.ErrorFilter)})
     * over a sliding 1-minute window. If a host had more than this number of errors, it will be
     * excluded from the query plan for the duration defined by {@link #withRetryPeriod(long,
     * TimeUnit)}.
     *
     * <p>Default value for the threshold is 1.
     *
     ...
     *
     * <p>Default value for the retry period is 2 minutes.
     *
     ...
     * #withMaxErrorsPerMinute(int)}.
     *
     * <p>The default implementation will exclude from the error counting, the following exception
     * types:
     *
     * <ul>
     *   <li>{@link QueryConsistencyException} and {@link UnavailableException}: the assumption is
     *       that these errors are most often caused by other replicas being unavailable, not by
     *       something wrong on the coordinator;
     *   <li>{@link InvalidQueryException}, {@link AlreadyExistsException}, {@link SyntaxError}:
     ...
     *       fix the query;
     *   <li>{@link FunctionExecutionException}: similarly, this is caused by a bad function
  ...
  class PerHostErrorTracker implements LatencyTracker {
    ...
    private final ConcurrentMap<Host, RollingCount> hostsCounts =
        new ConcurrentHashMap<Host, RollingCount>();
    ...
    private void excludeNow(Host host, long count, Long previousTime) {
      ...
      if (didNotRace && logger.isDebugEnabled()) {
        logger.debug(
            String.format(
                "Host %s encountered %d errors in the last minute, which is more "
    ...
    private RollingCount getOrCreateCount(Host host) {
      ...
      if (hostCount == null) {
        ...
        if (hostCount == null) hostCount = tmp;
      }
      return hostCount;
    }
  ...
  static class DefaultErrorFilter implements ErrorFilter {
    ...
    public boolean shouldConsiderError(Exception e, Host host, Statement statement) {
      for (Class<? extends Exception> ignoredException : IGNORED_EXCEPTIONS) {
        if (ignoredException.isInstance(e)) return false;
      }
      return true;
    }
  }
   ...
   *
   * <p>Only errors that indicate something wrong with a host should lead to its exclusion from
   * query plans.
   */
