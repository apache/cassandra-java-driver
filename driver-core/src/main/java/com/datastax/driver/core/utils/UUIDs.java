 ...
 *         <li>Otherwise, if a native call to {@link Native#processId() getpid()} is possible, then
 *             the PID will be read from that call;
  ...
  private static String getProcessPiece() {
    ...
    if (pid == null && Native.isGetpidAvailable()) {
      try {
        ...
        if (pid == 0) {
          LOGGER.warn(
              "PID returned through native call was 0, JNR versions incompatible?  Falling back to JMX.");
          pid = null;
        } else {
    ...
    try {
      ...
      if (en != null) {
        while (en.hasMoreElements()) {
          ...
          while (enumIpAddr.hasMoreElements()) allIps.add(enumIpAddr.nextElement().toString());
        }
      }
