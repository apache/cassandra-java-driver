...
public final class UUIDs {
    ...
    /**
     * Formats the least-significant bits (clock-seq and node) for type-1
     * UUIDs based on constant, non-mac-address bits computed from the MD5
     * sum of the UTF8-encoded bytes of the supplied migration keyword.
     * <p/>
     * This method is useful in migration scenarios where you can GUARANTEE
     * that no UUID overlap will occur based solely on the timestamp, and
     * where you want the UUIDs to be exactly the same (for the same
     * timestamp), regardless of the machine on which the UUID is
     * constructed.  When migrating records with time-based UUIDs in the
     * primary key to Cassandra, it is useful for the migration to be
     * idempotent, and not dependent on which node the migration code ran
     * on.
     *
     * @param migrationKeyword the string to use to generate clock-seq and
     *                         node bits for the least-significant UUID bits
     * @return bits to use as the least-significant bits in a time-based UUID
     */
    public static long makeConstantClockSeqAndNode(String migrationKeyword) {
      try {
        MessageDigest digest = MessageDigest.getInstance("MD5");
        byte[] sum = digest.digest(migrationKeyword.getBytes(StandardCharsets.UTF_8));
        byte[] uuidLsbBytes = new byte[] { sum[15], sum[14], sum[13], sum[12], sum[11], 0x01, sum[10], (byte) (0x80 | sum[9] & 0x3f) };
        long uuidLsb = 0x0000000000000000L;
        for (int i = 0; i < 8; i++) {
          uuidLsb |= (uuidLsbBytes[i] & 0x00000000000000FFL) << (i*8);
        }
        return uuidLsb;
      }
      catch (NoSuchAlgorithmException e) {
    ...
    /**
     * Creates a new time-based (version 1) UUID for use in data migration.
     * <p/>
     * UUID generated by this method are suitable for use with the
     * {@code timeuuid} Cassandra type. In particular the generated UUID
     * includes the supplied timestamp.
     * <p/>
     * The caller is responsible for ensuring that the combination of
     * {@code unixTimestamp} and {@code uniqueCounter} is actually unique.
     *
     * @param unixTimestamp the unix timestamp to base the UUID on (in milliseconds
     *                      since 1970-01-01 UTC)
     * @param uniqueCounter the counter to use in case timestamp is not unique;
     *                      MUST be less than 10000 (zero-based value, range: [0-9999])
     * @param uuidLsb       the least significant bytes to use, see
     *                      {@code makeConstantClockSeqAndNode} if you want
     *                      a constant value
     * @return a new time-based UUID for use in data migration.
     * @throws IllegalArgumentException if {@code uniqueCounter} is 10000 or greater
     */
    public static UUID timeBasedForMigration(long unixTimestamp, int uniqueCounter, long uuidLsb) {
      if (uniqueCounter < 0 || uniqueCounter >= 10000) {
        throw new IllegalArgumentException("unique counter must be >= 0 and < 10000");
      }
      return new UUID(makeMSB(fromUnixTimestamp(unixTimestamp) + uniqueCounter), uuidLsb);
    }
