...
public class ProtocolOptions {
  ...
  @VisibleForTesting volatile int maxSchemaAgreementWaitSeconds;
  ...
  /**
   * Creates a new {@code ProtocolOptions} instance using the provided port (without SSL nor
   * authentication).
   *
   * <p>This is a shortcut for {@code new ProtocolOptions(port, null, AuthProvider.NONE)}.
   *
   ...
   *
   * @return the protocol version in use. This might return {@code null} if a particular version
  ...
  public ProtocolVersion getProtocolVersion() {
    return manager == null || manager.connectionFactory == null
        ? null
        : manager.connectionFactory.protocolVersion;
  }

  /**
   * Returns the compression used by the protocol.
   *
   * <p>By default, compression is not used.
   *
   ...
   *
   * <p>Note that while this setting can be changed at any time, it will only apply to newly created
   * connections.
   *
   * @param compression the compression algorithm to use (or {@code Compression.NONE} to disable
   *     compression).
   * @return this {@code ProtocolOptions} object.
   * @throws IllegalStateException if the compression requested is not available. Most compression
  ...
  public ProtocolOptions setCompression(Compression compression) {
    if (compression != Compression.NONE && compression.compressor() == null)
      throw new IllegalStateException(
          "The requested compression is not available (some compression require a JAR to be found in the classpath)");
   ...
   *
   * @return the {@code SSLOptions} used by this cluster (set at the cluster creation time) or
   *     {@code null} if SSL is not in use.
   */
   ...
   *
   * @return the {@code AuthProvided} used by this cluster (set at the cluster creation time). If no
   *     authentication mechanism is in use (the default), {@code AuthProvided.NONE} will be
   *     returned.
   */
