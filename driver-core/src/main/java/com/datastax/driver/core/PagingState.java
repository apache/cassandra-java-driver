 ...
 *
 * <p>This object represents the next page to be fetched if the query is multi page. It can be saved
 * and reused later on the same statement.
 *
 * <p>The PagingState can be serialized and deserialized either as a String or as a byte array.
 *
...
public class PagingState {
  ...
  private PagingState(byte[] complete) {
    // Check the sizes in the beginning of the buffer, otherwise we cannot build the paging state
    // object
    ...
    if (pagingSize + hashSize != pagingStateBB.remaining()
        && pagingSize + hashSize + 2 != pagingStateBB.remaining()) {
    ...
    this.protocolVersion =
        (pagingStateBB.remaining() > 0)
            ? ProtocolVersion.fromInt(pagingStateBB.getShort())
            : ProtocolVersion.V2;
  }

  private byte[] hash(
      Statement statement, ProtocolVersion protocolVersion, CodecRegistry codecRegistry) {
    byte[] digest;
    ByteBuffer[] values;
    MessageDigest md;
