...
public abstract class Statement {
   ...
   *
   * <p>The serial consistency level is only used by conditional updates ({@code INSERT}, {@code
   * UPDATE} or {@code DELETE} statements with an {@code IF} condition). For those, the serial
   ...
   * ConsistencyLevel.LOCAL_SERIAL}. While {@code ConsistencyLevel.SERIAL} guarantees full
   * linearizability (with other SERIAL updates), {@code ConsistencyLevel.LOCAL_SERIAL} only
   * guarantees it in the local data center.
   *
   * <p>The serial consistency level is ignored for any query that is not a conditional update
   * (serial reads should use the regular consistency level for instance).
   *
   ...
   *
   * <p>See {@link #setSerialConsistencyLevel(ConsistencyLevel)} for more detail on the serial
   * consistency level.
   *
   * @return the serial consistency level for this query, or {@code null} if no serial consistency
   *     level has been specified (through {@link #setSerialConsistencyLevel(ConsistencyLevel)}). In
   *     the latter case, the default serial consistency level will be used.
   */
   ...
   *
   * <p>By default (that is unless you call this method), tracing is not enabled.
   *
   * @return this {@code Statement} object.
   */
   ...
   *
   * <p>The routing key is optional in that implementers are free to return {@code null}. The
   * routing key is an hint used for token-aware routing (see {@link
   * com.datastax.driver.core.policies.TokenAwarePolicy}), and if provided should correspond to the
   ...
   *
   * <p>Note that not all query specify on which keyspace they operate on, and so this method can
   * always return {@code null}. Firstly, some queries do not operate inside a keyspace: keyspace
   * creation, {@code USE} queries, user creation, etc. Secondly, even query that operate within a
   * keyspace do not have to specify said keyspace directly, in which case the currently logged in
   * keyspace (the one set through a {@code USE} query (or through the use of {@link
   ...
   * which is a per-keyspace property) and having this method return {@code null} (or even a bogus
   ...
   * com.datastax.driver.core.policies.Policies#getRetryPolicy} in the cluster configuration. This
   ...
   *     retry policy has been set through {@link #setRetryPolicy} (in which case the Cluster retry
   *     policy will apply if necessary).
   */
   ...
   *
   * <p>The fetch size controls how much resulting rows will be retrieved simultaneously (the goal
   * being to avoid loading too much results in memory for queries yielding large results). Please
   * note that while value as low as 1 can be used, it is *highly* discouraged to use such a low
   ...
   * version is in use (i.e. if you've force version 1 through {@link
   * Cluster.Builder#withProtocolVersion} or you use Cassandra 1.2), you will get {@link
   * UnsupportedProtocolVersionException} when submitting this statement for execution.
   *
   * @param fetchSize the fetch size to use. If {@code fetchSize &lte; 0}, the default fetch size
   *     will be used. To disable paging of the result set, use {@code fetchSize ==
   *     Integer.MAX_VALUE}.
   * @return this {@code Statement} object.
   */
   ...
   *
   * @return the fetch size for this query. If that value is less or equal to 0 (the default unless
   *     {@link #setFetchSize} is used), the default fetch size will be used.
   */
   ...
   *
   * <p>This feature is only available when version {@link ProtocolVersion#V3 V3} or higher of the
   * native protocol is in use. With earlier versions, calling this method has no effect.
   *
   * <p>The actual timestamp that will be used for this query is, in order of preference:
   *
   * <ul>
   *   <li>the timestamp specified directly in the CQL query string (using the {@code USING
   *       TIMESTAMP} syntax);
   *   <li>the timestamp specified through this method, if different from {@link Long#MIN_VALUE};
   *   <li>the timestamp returned by the {@link TimestampGenerator} currently in use, if different
   *       from {@link Long#MIN_VALUE}.
   * </ul>
   *
  ...
  /**
   * Overrides the default per-host read timeout ({@link SocketOptions#getReadTimeoutMillis()}) for
   * this statement.
   *
   ...
   *
   * @return the timeout. Note that a negative value means that the default {@link
   *     SocketOptions#getReadTimeoutMillis()} will be used.
   */
   ...
   *
   * <p>This will cause the next execution of this statement to fetch results from a given page,
   * rather than restarting from the beginning.
   *
   * <p>You get the paging state from a previous execution of the statement (see {@link
   * ExecutionInfo#getPagingState()}. This is typically used to iterate in a "stateless" manner
   ...
   * }</pre>
   *
   * <p>The paging state can only be reused between perfectly identical statements (same query
   * string, same bound parameters). Altering the contents of the paging state or trying to set it
   * on a different statement will cause this method to fail.
   *
   * <p>Note that, due to internal implementation details, the paging state is not portable across
   * native protocol versions (see the <a
   * href="http://datastax.github.io/java-driver/features/native_protocol">online documentation</a>
   * for more explanations about the native protocol). This means that {@code PagingState} instances
   * generated with an old version won't work with a higher version. If that is a problem for you,
   * consider using the "unsafe" API (see {@link #setPagingStateUnsafe(byte[])}).
   *
   ...
   *
   * <ul>
   *   <li>for a {@link BoundStatement}, the codec registry isn't actually needed, so it's always
   *       safe to use this method;
   *   <li>for a {@link SimpleStatement} or {@link BuiltStatement}, you can use this method if you
   ...
   * paging state (previously extracted with {@link ExecutionInfo#getPagingStateUnsafe()}. It won't
   ...
   *
   * <p>See {@link #isIdempotent()} for more explanations about this property.
   *
  ...
  /**
   * Returns this statement's outgoing payload. Each time this statement is executed, this payload
   * will be included in the query request.
   *
   * <p>This method returns {@code null} if no payload has been set, otherwise it always returns
   * immutable maps.
   *
   * <p>This feature is only available with {@link ProtocolVersion#V4} or above. Trying to include
  ...
  /**
   * Set the given outgoing payload on this statement. Each time this statement is executed, this
   * payload will be included in the query request.
   *
   * <p>This method makes a defensive copy of the given map, but its values remain inherently
   * mutable. Care should be taken not to modify the original map once it is passed to this method.
   *
   * <p>This feature is only available with {@link ProtocolVersion#V4} or above. Trying to include
   * custom payloads in requests sent by the driver under lower protocol versions will result in an
   * {@link com.datastax.driver.core.exceptions.UnsupportedFeatureException} (wrapped in a {@link
   ...
   *
   * <p>The calculated size may be overestimated by a few bytes, but is never underestimated. If the
   * size cannot be calculated, this method returns -1.
   *
   ...
   * </ol>
   *
   * <p>Configuring a specific host causes the configured {@link LoadBalancingPolicy} to be
