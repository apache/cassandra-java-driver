...
public class SocketOptions {
   ...
   *
   * <p>That default is of 5 seconds.
   */
   ...
   *
   * <p>That default is of 12 seconds so as to be slightly bigger that the default Cassandra
   * timeout.
   *
   ...
   *
   * <p>As the name implies, the connection timeout defines how long the driver waits to establish a
   ...
   *
   * <p>The default value is {@link #DEFAULT_CONNECT_TIMEOUT_MILLIS}.
   *
   ...
   *
   * <p>This defines how long the driver will wait for a given Cassandra node to answer a query.
   *
   * <p>Please note that this is not the maximum time a call to {@link Session#execute} may block;
   ...
   * <number_of_cassandra_hosts>} (though the total number of hosts tried for a given query also
   ...
   *
   * <p>Also note that for efficiency reasons, this read timeout is approximate: it has an accuracy
   ...
   *
   * <p>When setting this value, keep in mind the following:
   *
   * <ul>
   *   <li>it should be higher than the timeout settings used on the Cassandra side ({@code
   *       *_request_timeout_in_ms} in {@code cassandra.yaml}).
   *   <li>the read timeout is only approximate and only control the timeout to one Cassandra host,
   ...
   * </ul>
   *
   * <p>If you don't call this method, the default value is {@link #DEFAULT_READ_TIMEOUT_MILLIS}.
   *
   ...
   *
   * <p>By default, this option is not set by the driver. The actual value will be the default from
   ...
   *
   * <p>By default, this option is set to {@code true} (Nagle disabled).
   *
