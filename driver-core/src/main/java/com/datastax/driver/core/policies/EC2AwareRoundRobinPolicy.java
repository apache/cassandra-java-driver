...
public class EC2AwareRoundRobinPolicy implements LoadBalancingPolicy {
    ...
    private final ConcurrentMap<String, ConcurrentMap<String, CopyOnWriteArrayList<Host>>> perRegionLiveHosts =
            new ConcurrentHashMap<String, ConcurrentMap<String, CopyOnWriteArrayList<Host>>>();
    ...
    /**
     * Creates a new ec2 aware round robin policy given the name of
     * the local local region and local availability zone.
     * <p/>
     ...
     * <p/>
     * The policy created will ignore all hosts in remote regions. In other words,
     * this is equivalent to {@code new EC2AwareRoundRobinPolicy(localRegion, localAvailabilityZone, 0)}.
     *
    ...
    private void putHost(Host host) {
        ...
        if (region == null) {
            region = new ConcurrentHashMap<String, CopyOnWriteArrayList<Host>>();
        ...
        if (azList == null) {
            azList = new CopyOnWriteArrayList<Host>();
