 ...
 *
 * <p>In case of write timeouts or unexpected errors, this policy will always return {@link
 * com.datastax.driver.core.policies.RetryPolicy.RetryDecision#rethrow()} if the statement is deemed
 * non-idempotent (see {@link #isIdempotent(Statement)}).
 *
 * <p>For all other cases, this policy delegates the decision to the child policy.
 *
 * @deprecated As of version 3.1.0, the driver doesn't retry non-idempotent statements for write
...
public class IdempotenceAwareRetryPolicy implements RetryPolicy {
  ...
  public RetryDecision onReadTimeout(
      ...
      int nbRetry) {
    return childPolicy.onReadTimeout(
  ...
  public RetryDecision onWriteTimeout(
    ...
    if (isIdempotent(statement))
      return childPolicy.onWriteTimeout(
          statement, cl, writeType, requiredAcks, receivedAcks, nbRetry);
  ...
  public RetryDecision onRequestError(
    ...
    if (isIdempotent(statement)) return childPolicy.onRequestError(statement, cl, e, nbRetry);
    else return RetryDecision.rethrow();
  }
   ...
   *
   * <p>The current implementation inspects the statement's {@link Statement#isIdempotent()
   * idempotent flag}; if this flag is not set, then it inspects {@link
   * QueryOptions#getDefaultIdempotence()}.
   *
   * <p>Subclasses may override if they have better knowledge of the statement being executed.
   *
   * @param statement The statement to execute.
   * @return {@code true} if the given statement is idempotent, {@code false} otherwise
   */
