 ...
 *
 * <p>This policy is not datacenter aware and will include every known Cassandra host in its round
 * robin algorithm. If you use multiple datacenter this will be inefficient and you will want to use
 * the {@link DCAwareRoundRobinPolicy} load balancing policy instead.
 */
public class RoundRobinPolicy implements LoadBalancingPolicy {
   ...
   *
   * <p>This policy consider all nodes as local. This is generally the right thing to do in a single
   * datacenter deployment. If you use multiple datacenter, see {@link DCAwareRoundRobinPolicy}
   * instead.
   *
   ...
   *
   * <p>The returned plan will try each known host of the cluster. Upon each call to this method,
   * the {@code i}th host of the plans returned will cycle over all the hosts of the cluster in a
   * round-robin fashion.
   *
