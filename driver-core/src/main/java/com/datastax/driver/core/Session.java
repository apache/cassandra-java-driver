 ...
 *
 * <p>Each session maintains multiple connections to the cluster nodes, provides policies to choose
 ...
 *
 * <p>Session instances are thread-safe and usually a single instance is enough per application. As
...
public interface Session extends Closeable {
   ...
   *
   * <p>Please note first that most users won't need to call this method explicitly. If you use the
   ...
   *
   * <p>Session initialization consists in connecting the Session to the known Cassandra hosts (at
   ...
   *
   * <p>If the Cluster instance this Session depends on is not itself initialized, it will be
   * initialized by this method.
   *
   * <p>If the session is already initialized, this method is a no-op.
   *
   ...
   *
   * <p>This is a convenience method for {@code execute(new SimpleStatement(query))}.
   *
   ...
   *
   * <p>This is a convenience method for {@code execute(new SimpleStatement(query, values))}.
   *
   * @param query the CQL query to execute.
   ...
   * Executes the provided query.
   *
   * <p>This method blocks until at least some result has been received from the database. However,
   ...
   *
   * <p>This is a convenience method for {@code executeAsync(new SimpleStatement(query))}.
   *
   * @param query the CQL query to execute.
   ...
   *
   * <p>This is a convenience method for {@code executeAsync(new SimpleStatement(query, values))}.
   *
   * @param query the CQL query to execute.
   * @param values values required for the execution of {@code query}. See {@link
   *     SimpleStatement#SimpleStatement(String, Object...)} for more details.
   ...
   *
   * <p>Note that for queries that don't return a result (INSERT, UPDATE and DELETE), you will need
   ...
   *
   * <p>Please note that if the same CQL statement is prepared more than once, all calls to this
   ...
   *
   * <p>This method is equivalent to {@link #prepare(String)} except that it does not block but
   ...
   *
   * <p>This method has no particular effect if the session was already closed (in which case the
   * returned future will return immediately).
   *
   * <p>Note that this method does not close the corresponding {@code Cluster} instance (which holds
   ...
   *
   * <p>Note that this method returns true as soon as the closing of this Session has started but it
   ...
   *
   * <p>The returned object provides information on which hosts the session is connected to, how
   ...
   *
   * <p>This mostly exposes information on the connections maintained by a Session: which host it is
  ...
  interface State {
     ...
     * has been grabbed).
     *
     * <p>Please note that this method really returns the hosts for which the session currently
     ...
     *
     * <p>Note that this refers to <em>active</em> connections. The actual number of connections
     ...
     *
     * <p>When the load to a host decreases, the driver will reclaim some connections in order to
     ...
     *
     * <p>This corresponds to the number of queries that have been sent (by the session this is a
