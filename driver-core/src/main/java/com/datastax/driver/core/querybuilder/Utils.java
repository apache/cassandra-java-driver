...
abstract class Utils {
   ...
   *
   * <p>This method is used to convert identifiers provided by the client (through methods such as
   * getKeyspace(String)), to the format used internally by the driver.
   *
   * <p>We expect client-facing APIs to behave like cqlsh, that is: - identifiers that are
   ...
   *
   * <p>Copied from {@link Metadata#handleId(String)} to avoid making it public.
   */
  ...
  static StringBuilder joinAndAppendValues(
      StringBuilder sb, CodecRegistry codecRegistry, List<?> values, List<Object> variables) {
    for (int i = 0; i < values.size(); i++) {
      if (i > 0) sb.append(",");
  ...
  static StringBuilder appendValue(
      Object value, CodecRegistry codecRegistry, StringBuilder sb, List<Object> variables) {
    if (value == null) {
      sb.append("null");
   ...
   *
   * <p>This method is a copy of the one declared in {@link
   * com.datastax.driver.core.SimpleStatement}, it was duplicated to avoid having to make it public.
   *
   * <p>It is useful in situations where there is no metadata available and the underlying CQL type
   * for the values is not known.
   *
   * <p>This situation happens when a {@link com.datastax.driver.core.SimpleStatement} or a {@link
   ...
   *
   * <p>This method performs a best-effort heuristic to guess which codec to use. Note that this is
