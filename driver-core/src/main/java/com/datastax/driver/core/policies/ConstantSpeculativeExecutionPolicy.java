...
public class ConstantSpeculativeExecutionPolicy implements SpeculativeExecutionPolicy {
   ...
   *
   * @param constantDelayMillis the delay between each speculative execution. Must be >= 0. A zero
   *     delay means it should immediately send `maxSpeculativeExecutions` requests along with the
   *     original request.
   * @param maxSpeculativeExecutions the number of speculative executions. Must be strictly
   *     positive.
  ...
  public ConstantSpeculativeExecutionPolicy(
      final long constantDelayMillis, final int maxSpeculativeExecutions) {
    Preconditions.checkArgument(
        constantDelayMillis >= 0, "delay must be >= 0 (was %d)", constantDelayMillis);
    Preconditions.checkArgument(
