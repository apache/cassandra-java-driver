 ...
 *
 * <p>The driver auto-detect new Cassandra nodes added to the cluster through server side pushed
 * notifications and through checking the system tables. For each node, the address the driver will
 * receive will correspond to the address set as {@code rpc_address} in the node yaml file. In most
 ...
 * instance, the {@code rpc_address} set on Cassandra nodes might be a private IP, but some clients
 ...
 *
 * <p>Please note that the contact points addresses provided while creating the {@link Cluster}
