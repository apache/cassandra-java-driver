...
class CassandraTypeParser {
    ...
    static DataType parseOne(String className, ProtocolVersion protocolVersion, CodecRegistry codecRegistry) {
        ...
        if (isUserType(next)) {
            ...
            String typeName = TypeCodec.varcharCodec().deserialize(Bytes.fromHexString("0x" + parser.readOne()), protocolVersion);
            parser.skipBlankAndComma();
    ...
    private static class Parser {
        ...
        public Map<String, String> getNameAndTypeParameters() {
            ...
            while (skipBlankAndComma()) {
                ...
                String name = null;
                try {
                    name = TypeCodec.varcharCodec().deserialize(Bytes.fromHexString("0x" + bbHex), ProtocolVersion.NEWEST_SUPPORTED);
