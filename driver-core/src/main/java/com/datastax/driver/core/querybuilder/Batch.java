...
public class Batch extends BuiltStatement {
  ...
  Batch(RegularStatement[] statements, boolean logged) {
    ...
    for (int i = 0; i < statements.length; i++) add(statements[i]);
  }
   ...
   * @return this batch.
   * @throws IllegalArgumentException if counter and non-counter operations are mixed.
   */
  public Batch add(RegularStatement statement) {
    boolean isCounterOp =
        statement instanceof BuiltStatement && ((BuiltStatement) statement).isCounterOp();

    if (this.isCounterOp == null) setCounterOp(isCounterOp);
    ...
    if (statement instanceof BuiltStatement) {
      ...
      // For non-BuiltStatement, we cannot know if it includes a bind makers and we assume it does.
      // In practice,
