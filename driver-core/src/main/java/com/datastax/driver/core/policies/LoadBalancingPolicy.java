 ...
 *
 * <p>Two methods need to be implemented:
 *
 * <ul>
 *   <li>{@link LoadBalancingPolicy#distance}: returns the "distance" of an host for that balancing
 *       policy.
 *   <li>{@link LoadBalancingPolicy#newQueryPlan}: it is used for each query to find which host to
...
public interface LoadBalancingPolicy {
   ...
   * and will do so before any call to another of the methods of the policy.
   *
   ...
   *
   * <p>The distance of an host influence how much connections are kept to the node (see {@link
   * HostDistance}). A policy should assign a {@code LOCAL} distance to nodes that are susceptible
   * to be returned first by {@code newQueryPlan} and it is useless for {@code newQueryPlan} to
   * return hosts to which it assigns an {@code IGNORED} distance.
   *
   * <p>The host distance is primarily used to prevent keeping too many connections to host in
   * remote datacenters when the policy itself always picks host in the local datacenter first.
   *
   ...
   *
   * <p>Each new query will call this method. The first host in the result will then be used to
   * perform the query. In the event of a connection problem (the queried host is down or appear to
   * be so), the next host will be used. If all hosts of the returned {@code Iterator} are down, the
   * query will fail.
   *
   ...
   * @return an iterator of Host. The query is tried against the hosts returned by this iterator in
   *     order, until the query has been sent successfully to one of the host.
   */
