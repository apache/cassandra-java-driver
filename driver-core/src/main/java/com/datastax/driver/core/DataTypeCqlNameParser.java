...
class DataTypeCqlNameParser {
  ...
  /**
   * @param currentUserTypes if this method gets called as part of a refresh that spans multiple
   *     user types, this contains the ones that have already been refreshed. If the type we are
   *     parsing references a user type, we want to pick its definition from this map in priority.
   * @param oldUserTypes this contains all the keyspace's user types as they were before the refresh
   *     started. If we can't find a definition in {@code currentUserTypes}, we'll check this map as
   *     a fallback.
   */
  static DataType parse(
      String toParse,
      Cluster cluster,
      String currentKeyspaceName,
      Map<String, UserType> currentUserTypes,
      Map<String, UserType> oldUserTypes,
      boolean frozen,
      boolean shallowUserTypes) {

    if (toParse.startsWith("'")) return custom(toParse.substring(1, toParse.length() - 1));
  ...
  private static class Parser {
    ...
    List<String> parseTypeParameters() {
      ...
      while (skipBlankAndComma()) {
        ...
        try {
          ...
          DriverInternalError ex =
              new DriverInternalError(
                  String.format("Exception while parsing '%s' around char %d", str, idx));
          ex.initCause(e);
          throw ex;
        }
      }
    ...
    private String readRawTypeParameters() {
      ...
      while (open > 0) {
        ...
        if (isEOS()) throw new IllegalStateException("Non closed angle brackets");
