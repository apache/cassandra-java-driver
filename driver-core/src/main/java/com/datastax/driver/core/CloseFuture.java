 ...
 *
 * <p>This is a standard future except for the fact that this class has an additional {@link #force}
...
public abstract class CloseFuture extends AbstractFuture<Void> {
   ...
   *
   * <p>This method will do its best to expedite the shutdown process. In particular, all
   * connections will be closed right away, even if there are ongoing queries at the time this
   * method is called.
   *
   * <p>Note that this method does not block. The completion of this method does not imply the
   * shutdown process is done, you still need to wait on this future to ensure that, but calling
   * this method will ensure said future will return in a timely way.
   *
  ...
  static class Forwarding extends CloseFuture {
    ...
    @Override
    public CloseFuture force() {
      for (CloseFuture future : futures) future.force();
      return this;
    }
