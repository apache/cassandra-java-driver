...
The only visible effect is that a request may fail with a https://docs.datastax.com/en/drivers/java/4.14/com/datastax/oss/driver/api/core/RequestThrottlingException.html[RequestThrottlingException], if the throttler has determined that it can neither allow the request to proceed now, nor enqueue it;
...
the driver should never run out of stream ids before reaching the maximum concurrency, otherwise requests will fail with https://docs.datastax.com/en/drivers/java/4.14/com/datastax/oss/driver/api/core/connection/BusyConnectionException.html[BusyConnectionException] instead of being throttled.
