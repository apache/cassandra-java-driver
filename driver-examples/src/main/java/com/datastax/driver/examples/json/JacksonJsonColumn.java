 ...
 * }</pre>
 *
 * This example also uses the {@link com.datastax.driver.core.querybuilder.QueryBuilder
 ...
 * already exists, it will be reused; - creates a table "examples.json_jackson_column". If it
 * already exists, it will be reused; - inserts data in the table.
 */
public class JacksonJsonColumn {
  ...
  private static void selectJsonColumn(Session session) {
    ...
    Statement stmt = select().from("examples", "json_jackson_column").where(in("id", 1, 2));
