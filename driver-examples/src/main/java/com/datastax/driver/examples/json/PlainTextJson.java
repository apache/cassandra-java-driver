 ...
 * points identified by CONTACT_POINTS and PORT;
 *
...
public class PlainTextJson {
  ...
  private static void insertWithCoreApi(Session session) {
    ...
    session.execute(
        "INSERT INTO examples.querybuilder_json " + "(id, name, specs)  VALUES (?, ?, fromJson(?))",
        3,
        "Screen",
        "{ \"size\": \"24-inch\" }");
  }
  ...
  private static void selectWithCoreApi(Session session) {
    ...
    Row row =
        session.execute("SELECT JSON * FROM examples.querybuilder_json WHERE id = ?", 1).one();
  ...
  private static void insertWithQueryBuilder(Session session) {
    ...
    PreparedStatement pst =
        session.prepare(insertInto("examples", "querybuilder_json").json(bindMarker("payload")));
    session.execute(
        pst.bind()
            .setString(
                "payload",
  ...
  private static void selectWithQueryBuilder(Session session) {
    // Reading the whole row as a JSON object:
    Statement stmt = select().json().from("examples", "querybuilder_json").where(eq("id", 1));
