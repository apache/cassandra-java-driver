...
However, the routing keyspace is only used for https://docs.datastax.com/en/drivers/java/4.14/com/datastax/oss/driver/api/core/session/Request.html#getRoutingKey--[token-aware routing], as a hint to help the driver send requests to the best replica.
